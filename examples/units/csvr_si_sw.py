...
# %% [markdown]
# [![Download Notebook](https://img.shields.io/badge/Download-Notebook-blue?style=for-the-badge&logo=jupyter)](https://jax-md.readthedocs.io/en/main/notebooks/units/csvr_si_sw.ipynb)
# [![Download Python Script](https://img.shields.io/badge/Download-Python_Script-green?style=for-the-badge&logo=python)](https://raw.githubusercontent.com/google/jax-md/main/examples/units/csvr_si_sw.py)

# %% [markdown]
# # Metal Units (CSVR Thermostat)
#
# This notebook demonstrates the use of a unit system (metal units) for the simulation of the Silicon crystal containing 512 atoms with CSVR (canonical sampling through velocity rescaling) thermostat and the Stillinger-Weber potential. This notebook use lammps velocities and positions as a starting point for the simulation and for comparison.
#
...
base_url = 'https://raw.githubusercontent.com/abhijeetgangan/silicon_data/main/Si_FF/Si_SW_MD/NVT_CSVR_300K/'
download_file(base_url + 'lammps_nvt.dat', 'csvr_nvt.dat')
...
data_lammps = pd.read_csv('csvr_nvt.dat', delim_whitespace=True, header=None)
...
NSTEPS_SIM = 1000 if SMOKE_TEST else 200000
...
# CSVR simulation
init_fn, apply_fn = simulate.temp_csvr(energy_fn, shift, dt=dt, kT=T_init, tau=100 * dt)
