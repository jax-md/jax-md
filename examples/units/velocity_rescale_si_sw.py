...
# %% [markdown]
# [![Download Notebook](https://img.shields.io/badge/Download-Notebook-blue?style=for-the-badge&logo=jupyter)](https://jax-md.readthedocs.io/en/main/notebooks/units/velocity_rescale_si_sw.ipynb)
# [![Download Python Script](https://img.shields.io/badge/Download-Python_Script-green?style=for-the-badge&logo=python)](https://raw.githubusercontent.com/google/jax-md/main/examples/units/velocity_rescale_si_sw.py)

# %% [markdown]
# # Metal Units (Velocity Rescaling)
#
# This notebook demonstrates the use of a unit system (metal units) for the simulation of the Silicon crystal containing 512 atoms with velocity rescaling thermostat and the Stillinger-Weber potential. This notebook use lammps velocities and positions as a starting point for the simulation and for comparison.
#
...
base_url = 'https://raw.githubusercontent.com/abhijeetgangan/silicon_data/main/Si_FF/Si_SW_MD/NVT_explicit_300K/'
download_file(base_url + 'lammps_nvt.dat', 'velocity_rescale_nvt.dat')
...
data_lammps = pd.read_csv('velocity_rescale_nvt.dat', delim_whitespace=True, header=None)
...
# Velocity rescaling simulation
init_fn, apply_fn = simulate.temp_rescale(
  energy_fn, shift, dt=dt, kT=T_init, window=15 * unit['temperature'], fraction=0.5
)
...
def outer_sim_fn(j, state_nbrs_log):
  ...
  debug.print('Step = {j} | Total Energy = {T} | Temp = {Temp}', j=j * write_every, T=E, Temp=kT / unit['temperature'])
...
if data_lammps is not None:
  ax1.plot(t_l[:NSTEPS], T_l[:NSTEPS], lw=2, label='LAMMPS')
...
if data_lammps is not None:
  ax2.plot(t_l[:NSTEPS], P_l[:NSTEPS] / 10000, lw=2, label='LAMMPS')
...
if data_lammps is not None:
  ax3.plot(t_l[:NSTEPS], E_l[:NSTEPS], lw=2, label='LAMMPS')
...
#
# Compare the distribution of potential energies between JAX-MD and LAMMPS
...
if data_lammps is not None:
  lammps_energy = onp.array(E_l[NSKIP:NSTEPS] / 512)
