...
def optimized_opls_aa_energy_with_nlist_modular(
  ...
  def make_is_14_lookup(pair_indices, molecule_id, is_14_mask, num_atoms):
    same_mol = (
      molecule_id[pair_indices[:, 0]] == molecule_id[pair_indices[:, 1]]
    )
  ...
  def make_exclusion_mask(num_atoms, bond_idx, angle_idx, molecule_id):
    ...
    angle_same_mol = (
      molecule_id[angle_idx[:, 0]] == molecule_id[angle_idx[:, 2]]
    )
    bond_idx_filtered = bond_idx[bond_same_mol]
    angle_idx_filtered = angle_idx[angle_same_mol]
    exclusion_mask = exclusion_mask.at[
      bond_idx_filtered[:, 0], bond_idx_filtered[:, 1]
    ].set(True)
    exclusion_mask = exclusion_mask.at[
      bond_idx_filtered[:, 1], bond_idx_filtered[:, 0]
    ].set(True)
    exclusion_mask = exclusion_mask.at[
      angle_idx_filtered[:, 0], angle_idx_filtered[:, 2]
    ].set(True)
    exclusion_mask = exclusion_mask.at[
      angle_idx_filtered[:, 2], angle_idx_filtered[:, 0]
    ].set(True)
  ...
  def get_masks(num_atoms):
    ...
    if table_14 is None:
      table_14 = make_is_14_lookup(
        pair_indices, molecule_id, is_14_mask, num_atoms
      )
  ...
  def bonded_and_lj_energy(positions, nlist):
    ...
    # Bonded terms
    r_bond = vmap(
      lambda i, j: safe_norm(displacement_fn(positions[i], positions[j]))
    ...
    E_torsion = jnp.sum(
      k_torsion * (1 + jnp.cos(n_torsion * phi - gamma_torsion))
    )
    ...
    E_improper = jnp.sum(
      k_improper * (1 + jnp.cos(n_improper * psi - gamma_improper))
    )
