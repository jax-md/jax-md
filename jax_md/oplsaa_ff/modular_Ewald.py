...
class CutoffCoulomb(CoulombHandler):
  ...
  def energy(self, positions, charges, box, exclusion_mask, is_14_table, nlist):
    ...
    idx_i, idx_j = jnp.meshgrid(
      jnp.arange(num_atoms), jnp.arange(num_atoms), indexing='ij'
    )
    ...
    same = idx_i == idx_j
    excluded = vmap(lambda i_row, j_row: exclusion_mask[i_row, j_row])(
      idx_i, idx_j
    )
...
class EwaldCoulomb(CoulombHandler):
  ...
  def real_energy(
    ...
    idx_i = jnp.repeat(
      jnp.arange(num_atoms)[:, None], max_neighbors, axis=1
    )  # (N, M)
    idx_j = nlist.idx  # (N, M)
    ...
    excluded = jax.vmap(lambda i_row, j_row: exclusion_mask[i_row, j_row])(
      idx_i_safe, idx_j_safe
    )
    is_14 = jax.vmap(lambda i_row, j_row: is_14_table[i_row, j_row])(
      idx_i_safe, idx_j_safe
    )

    # Batched displacement
    batched_displacement = jax.vmap(
      jax.vmap(displacement_fn, in_axes=(0, 0)), in_axes=(0, 0)
    )
    ...
    safe_energy_raw = (
      self.prefactor * qi * qj / r_safe * (erfc_val - (1.0 - factor_coul))
    )
  ...
  def energy(self, positions, charges, box, exclusion_mask, is_14_table, nlist):
    ...
    e_real = self.real_energy(
      positions, charges, displacement_fn, exclusion_mask, is_14_table, nlist
    )
...
class PME_Coulomb(CoulombHandler):
  ...
  def structure_factor(self, charges, positions, box):
    ...
    def deposit_single(charge, base, frac):
      ...
      grid = jnp.zeros(
        (self.grid_size, self.grid_size, self.grid_size), dtype=jnp.complex64
      )
...
def make_is_14_lookup(pair_indices, is_14_mask, num_atoms):
  ...
  is_14_table = is_14_table.at[pair_indices[:, 0], pair_indices[:, 1]].set(
    is_14_mask
  )
  is_14_table = is_14_table.at[pair_indices[:, 1], pair_indices[:, 0]].set(
    is_14_mask
  )
  return is_14_table
