...
def calculate_reaxff_energy(
    ...
    atom_mask = species >= 0
    self_energy = (
        jnp.sum(force_field.self_energies[species] * atom_mask) + force_field.shift[0]
    )
    ...
    tapered_dists = jnp.where(
        (far_nbr_dists > 10.0) | (far_nbr_dists < 0.001), 0.0, tapered_dists
    )
    ...
    if solver_model == "EEM":
        ...
        bond_softness = safe_mask(far_nbr_dists < xcut, lambda x: x, bond_softness, 0.0)
        ...
        charge_pot_acks2 = calculate_charge_energy_acks2(charges[:-1], effpot)

        coulomb_acks2 = calculate_acks2_coulomb_pot(
            far_nbr_inds, atom_mask, effpot, bond_softness
        )
    result_dict["charges"] = charges[:-1]

    cou_pot = calculate_coulomb_pot(
        far_nbr_inds, atom_mask, hulp2_mat, tapered_dists, charges[:-1]
    )
    cou_pot += coulomb_acks2
    result_dict["E_coulomb"] = cou_pot

    charge_pot = calculate_charge_energy(
        species, charges[:-1], force_field.idempotential, force_field.electronegativity
    )
    charge_pot += charge_pot_acks2
    result_dict["E_charge"] = charge_pot

    vdw_pot = calculate_vdw_pot(
        species, far_nbr_mask, far_nbr_inds, far_nbr_dists, tapered_dists, force_field
    )
    ...
    covbon_mask = (close_nbr_inds != N) & (
        atom_mask.reshape(-1, 1) & atom_mask[close_nbr_inds]
    )
    [cov_pot, bo, bopi, bopi2, abo] = calculate_covbon_pot(
        close_nbr_inds, close_nbr_dists, covbon_mask, species, triple_bond, force_field
    )
    result_dict["E_covalent"] = cov_pot

    [lone_pot, vlp] = calculate_lonpar_pot(species, atom_mask, abo, force_field)

    result_dict["E_lone_pair"] = lone_pot
    overunder_pot = calculate_ovcor_pot(
        species,
        atomic_numbers,
        atom_mask,
        close_nbr_inds,
    ...
    [val_pot, total_penalty, total_conj] = calculate_valency_pot(
        species,
        body_3_inds,
        body_3_angles,
        body_3_inds[:, 0] != -1,
    ...
    [torsion_pot, tor_conj] = calculate_torsion_pot(
        species,
        body_4_inds,
        body_4_angles,
        body_4_inds[:, 0] != -1,
...
def calculate_eem_charges(
    ...
    def to_dense():
        ...
        """
        A_ = jax.vmap(
            lambda j: jax.vmap(lambda i: jnp.sum(A[i] * (nbr_inds[i] == j)))(
                jnp.arange(N)
            )
        )(jnp.arange(N))
    ...
    def SPMV_dense(vec):
        ...
        vals = jax.vmap(jnp.dot)(A, s_vec) + (my_idemp * 2.0) * vec[:N] + vec[N]
    ...
    if max_solver_iter == -1:
        ...
        charges, conv_info = linalg.cg(
            SPMV_dense, b, x0=init_charges, tol=tol, maxiter=max_solver_iter
        )
...
def calculate_coulomb_pot(
    ...
    eph_mat = safe_mask(mask, lambda x: c1c * charge_mat / (x + 1e-20), hulp2_mat, 0.0)
...
def calculate_charge_energy(
    species: Array, charges: Array, idempotential: Array, electronegativity: Array
):
    ech = high_precision_sum(
        23.02
        * (
            electronegativity[species] * charges
            + idempotential[species] * jnp.square(charges)
        )
    )
    return ech


def calculate_acks2_coulomb_pot(
    nbr_inds: Array, atom_mask: Array, effpot: Array, bond_softness: Array
):
...
def calculate_charge_energy_acks2(charges: Array, effpot: Array):
    ech_acks2 = high_precision_sum(23.02 * charges * effpot)
    return ech_acks2


def calculate_vdw_pot(
    species: Array,
    far_nbr_mask: Array,
    nbr_inds: Array,
    dists: Array,
    ...
    hulpw_mat = (
        safe_mask(dists > 0, lambda x: x**force_field.vdw_shiedling, dists, 0.0)
        + gamwco_mat
    )
...
def calculate_bo(
    ...
    [cov_pot, bo, bopi, bopi2, abo] = calculate_covbon_pot(
        nbr_inds, nbr_dist, nbr_inds != N, species, triple_bond, force_field
    )

    return bo


def calculate_covbon_pot(
    nbr_inds: Array,
    nbr_dist: Array,
    nbr_mask: Array,
    species: Array,
    ...
    bo, bopi, bopi2 = calculate_boncor_pot(
        nbr_inds, nbr_mask, species.flatten(), bo, bopi, bopi2, abo, force_field
    )
...
def calculate_boncor_pot(
    ...
    vval3 = jnp.where(force_field.amas < 21.0, force_field.valf, force_field.vval3)
...
def calculate_lonpar_pot(
    species: Array, atom_mask: Array, abo: Array, force_field: ForceField
):
...
def calculate_valency_pot(
    ...
    sbo2 = vectorized_cond(
        sbo2 < 1, lambda x: (x + 1e-15) ** force_field.val_par17, lambda x: sbo2, sbo2
    )
...
def calculate_torsion_pot(
    ...
    my_v1 = force_field.v1[
        species[left_glb],
        species[center1_glb],
        species[center2_glb],
        species[right_glb],
    ]
    my_v2 = force_field.v2[
        species[left_glb],
        species[center1_glb],
        species[center2_glb],
        species[right_glb],
    ]
    my_v3 = force_field.v3[
        species[left_glb],
        species[center1_glb],
        species[center2_glb],
        species[right_glb],
    ]
    my_v4 = force_field.v4[
        species[left_glb],
        species[center1_glb],
        species[center2_glb],
        species[right_glb],
    ]
    my_vconj = force_field.vconj[
        species[left_glb],
        species[center1_glb],
        species[center2_glb],
        species[right_glb],
...
def calculate_hb_pot(
    ...
    ehbh = (
        (
            ...
            * my_dehb
            * exphu2
            * jnp.power(jnp.sin((angles + 1e-10) / 2.0), 4)
        )
        * boa_mult
        * dist_mult
    )
