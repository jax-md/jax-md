...
def main(unused_argv):
    ...
    amorphous_structure = mg.core.Structure(
        lattice=box.T, species=random_packed_structure.species, coords=state.position
    )
    ...
    def relax_structure(s, n_steps=500, alpha=0.05):
        ...
        log = {"energy": jnp.zeros((n_steps,)), "stress": jnp.zeros((n_steps, 3, 3))}
        state = fire_init(R)
        ...
        (
            ...
            jnp.sum(jnp.diagonal(quantity.stress(energy_fn, state.position, box))) / 3,
        )
        return (
            mg.core.Structure(lattice=box.T, species=s.species, coords=state.position),
            log,
            final_e,
            final_p,
        )
    ...
    lowest_e_struct = sorted(relaxed_structures, key=lambda x: x[-2] / x[0].num_sites)[
        0
    ]
