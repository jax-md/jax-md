...
def _estimate_cell_capacity(
    position: Array, box_size: Box, cell_size: float, buffer_size_multiplier: float
) -> int:
...
def cell_list(
    ...
    def cell_list_fn(
        ...
        **kwargs,
        ...
        return CellList(
            cell_position, cell_id, cell_kwargs, overflow, cell_capacity, update_fn
        )  # pytype: disable=wrong-arg-count

    def allocate_fn(position: Array, extra_capacity: int = 0, **kwargs) -> CellList:
    ...
    for dim in range(1, 4):
        try:
            ...
            if len(dR_or_dr.shape) == 0:
                return (
                    lambda Ra, Rb, **kwargs: displacement_or_metric(Ra, Rb, **kwargs)
                    ** 2
                )
            else:
...
def neighbor_list(
    displacement_or_metric: DisplacementOrMetricFn,
    box_size: Optional[Box],
    r_cutoff: float,
    dr_threshold: float,
    capacity_multiplier: float = 1.25,
    disable_cell_list: Optional[bool] = None,
    mask_self: bool = True,
    ...
    **static_kwargs,
    ...
    Args:
      ...
      fractional_coordinates: An optional boolean. Specifies whether positions
        ...
        :math:`[0, 1]^d`. If this is set to True then the `box_size` will be set
        to `1.0` and the cell size used in the cell list will be set to
        `cutoff / box_size`.
      format: The format of the neighbor list; see the :meth:`NeighborListFormat`
        enum for details about the different choices for formats. Defaults to
        `Dense`.
    ...
    if disable_cell_list is not None:
        logging.warning(
            "`disable_cell_list` will be depricated. Please set "
            "`box_size=None` to disable cell lists."
        )
        box_size = None if disable_cell_list else box_size
        use_cell_list = False
    ...
    if box_size is not None:
        ...
        use_cell_list = jnp.all(cell_size < box_size / 3.0)
        if use_cell_list:
    ...
    def allocate_fn(position: Array, extra_capacity: int = 0, **kwargs) -> NeighborList:
        return neighbor_list_fn(position, extra_capacity=extra_capacity, **kwargs)

    def update_fn(position: Array, neighbors: NeighborList, **kwargs) -> NeighborList:
