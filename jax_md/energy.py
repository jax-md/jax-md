...
def simple_spring(
    dr: Array, length: Array = 1, epsilon: Array = 1, alpha: Array = 2, **unused_kwargs
) -> Array:
...
def soft_sphere(
    dr: Array, sigma: Array = 1, epsilon: Array = 1, alpha: Array = 2, **unused_kwargs
) -> Array:
    """.. _soft-sphere:
...
def gupta_gold55(displacement, cutoff=8.0):
    gupta_gold_fn = gupta_potential(displacement, cutoff=cutoff, **GUPTA_GOLD55_DICT)
...
def bks_silica_pair(
    displacement_or_metric: DisplacementOrMetricFn, species: Array, cutoff: float = 8.0
):
...
def _sw_radial_interaction(sigma: float, B: float, cutoff: float, r: Array) -> Array:
...
def stillinger_weber_neighbor_list(
    ...
    **neighbor_kwargs
    ...
    neighbor_fn = partition.neighbor_list(
        displacement, box_size, cutoff, dr_threshold, format=format, **neighbor_kwargs
    )
...
def _ters_cutoff(dr, R, D) -> Array:
    ...
    Returns:
      cut-off values
    """
    outer = jnp.where(dr < R + D, 0.5 * (1.0 - jnp.sin(jnp.pi / 2 * (dr - R) / D)), 0)
...
def tersoff(
    ...
    def compute_fn(R, **kwargs):
        ...
        second_term = util.high_precision_sum(attractive_fn(dR, dR, mask_ijk) * mask)
    ...
    **neighbor_kwargs
    ...
    def energy_fn(R, neighbor, **kwargs):
        ...
        if neighbor.format is partition.Dense:
            ...
            N = len(R)
            dr = space.map_bond(metric)(
                R[neighbor.idx[0]], R[neighbor.idx[1]], **kwargs
            )
...
def behler_parrinello(
    ...
    sym_fn = nn.behler_parrinello_symmetry_functions(
...
def behler_parrinello_neighbor_list(
    ...
    format: partition.NeighborListFormat = partition.Sparse,
    **neighbor_kwargs
) -> Tuple[NeighborFn, nn.InitFn, Callable[[PyTree, Array, NeighborList], Array]]:
    if sym_kwargs is None:
        sym_kwargs = {}
    if mlp_kwargs is None:
        mlp_kwargs = {"activation": jnp.tanh}
