...
def pair(
    ...
    **kwargs,
    ...
    if species is None:
        ...
        raise ValueError(
            "Species must be None, an ndarray, or an integer. Found {}.".format(species)
        )
    return fn_mapped
...
def _get_neighborhood_species_params(
    format: partition.NeighborListFormat, idx: Array, species: Array, params: Parameter
) -> Parameter:
    """Get parameters for interactions between species pairs."""
    ...
    **kwargs,
    ...
    if species is None:
        ...
        def fn_mapped(R, **dynamic_kwargs) -> Array:
            ...
            return high_precision_sum(output, axis=reduce_axis, keepdims=keepdims) / 2.0
    elif util.is_array(species):
