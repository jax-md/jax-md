...
class TPUGrid:
  ...
  box_size: float = dataclasses.static_field()
...
def to_grid(positions: Array,
  ...
  cell_data = _positions_to_grid(positions, box_size, cell_size, ids + 1, aux)    
  ...
  if topology:
    move_fn = parallelize(move_fn, topology)
...
def from_grid(grid: TPUGrid, aux: Optional[PyTree] = None
  ...
  grid_centers = _grid_centers(box_size, cell_size, num_dims)
...
def _grid_centers(box_size: float, cell_size: float, num_dims: int) -> Array:
  """Computes the center position of each grid cell."""
  cells_count_per_side = int(onp.ceil(box_size / cell_size))
  grid_centers = onp.zeros((cells_count_per_side,) * num_dims + (num_dims,))
  cell_idx = onp.arange(cells_count_per_side)
  for i in range(num_dims):
    new_shape = (1,) * i + (cells_count_per_side,) + (1,) * (num_dims - i - 1)
...
def cell_hash(cell_index, cells, num_dims):
  if num_dims == 1:
    ...
    return cell_index[:, 1] + cell_index[:, 0] * cells
  elif num_dims == 3:
    return cell_index[:, 2] + cell_index[:, 1] * cells + cell_index[:, 0] * cells ** 2
  else:
    raise ValueError() # TODO: Make this error message nicer.
...
def _positions_to_grid(position: Array,
  ...
  if strategy == 'linear':
    ...
    centers = _grid_centers(box_size, cell_size, num_dims).reshape((-1, num_dims))
    ...
    cell_contents = np.concatenate([cell_contents, particle_id[:, None]], axis=-1)
    if aux is not None:
      cell_contents, *_ = _set_aux(cell_contents, aux)
    grid = np.zeros((cells ** num_dims, cell_contents.shape[-1]))
...
def _folded_pad(data: Array,
  ...
  for i in range(num_dims):
    pre = (0,) * i
    post = (0,) * (data.ndim - i - 1)
    all_slice = tuple(slice(data.shape[j]) for j in range(i + num_dims))
    p1_idx = all_slice + (slice(-pad_size, data.shape[i + num_dims]),)
    p1 = np.roll(data[p1_idx], pre + (1,) + post, axis=range(data.ndim))
    p2_idx = all_slice + (slice(0, pad_size),)
    p2 = np.roll(data[p2_idx], pre + (-1,) + post, axis=range(data.ndim))
    data = np.concatenate([p1, data, p2], axis=num_dims + i)
  return data
...
def _fold_factors(batch_size: int,
  ...
  factors = 2 ** folds
  if onp.sum(folds) < max_folds:
    print(f'Folds {folds} have sum smaller than target of {max_folds}. This '
          f'corresponds to a batch size of {onp.prod(factors)} rather than '
          f'the target of {batch_size}.')
  return factors
