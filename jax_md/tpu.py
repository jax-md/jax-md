...
def accumulate_on_grid(
    ...
    for dim in range(grid.num_dims):
        data = _pad_axis_channel_last(data, grid.factors, dim, grid.max_grid_distance)
...
def soft_sphere(
    ...
    """
    return pair_potential(energy.soft_sphere, sigma=sigma, epsilon=epsilon, alpha=alpha)
...
def _pad_axis(data: Array, factors: Tuple[int, ...], axis: int, padding: int) -> Array:
...
def _folded_pad(data: Array, max_grid_distance: int, factors: Tuple[int, ...]) -> Array:
...
def _unfold_grid(cell_data: Array, grid: TPUGrid, inner_fold: bool = True) -> Array:
...
def _mesh_transport(cell_data: Array, grid: TPUGrid) -> Array:
    ...
    cell_data = _fold_grid(cell_data, grid.max_grid_distance, factors=grid.factors)[0]
...
def _pairwise_exchange(cell_data: Array, axis: int, grid: TPUGrid) -> Array:
    ...
    axes_post = axes
    cell_data = einops.rearrange(cell_data, f"{axes_pre} c -> q {axes_post} c", q=2)
    ...
    def square_displacement(dat: Array) -> Array:
        return np.sum(
            dat[..., :num_dims] ** 2 * (dat[..., [-1]] > 0), axis=(0, -1), keepdims=True
        )
...
def _update_grid_locations(cell_data: Array, grid: TPUGrid) -> Array:
    ...
    for axis in range(grid.num_dims):
        ...
        keep_mask = _pad_axis_channel_last(keep_mask[body_slice], grid.factors, axis)
...
def _set_aux(cell_data: Array, aux: PyTree) -> Tuple[Array, TreeDef, Tuple[int, ...]]:
...
def _get_aux_spec(num_dims: int, aux: PyTree) -> Tuple[TreeDef, Tuple[int, ...]]:
