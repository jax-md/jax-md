...
Helper functions and types to iteratively solve holonomic constraints of the
form \sigma(t, \vec r) = 0 at each step of a simulation. This can be used
to e.g. simulate molecules or other systems with flexible bonds between
constituents.
"""
...
class HolonomicConstraint:
    constraint_fn: Callable = dataclasses.static_field()
    method: str = dataclasses.static_field()

    def __call__(self, R, **kwargs):
        return self.constraint_fn(R, **kwargs)


def initialize_lagrange_multipliers(constraint, state):
    if constraint is None:
        return state

    out_shape = eval_shape(constraint, state.position).shape
    assert "lagrange_multiplier" not in state.aux
    return state.set(
        aux=frozendict({"lagrange_multiplier": jnp.zeros(out_shape), **state.aux})
    )


def count_dof(state):
    lagrange_multiplier = state.aux.get("lagrange_multiplier", None)
    constraints = 0 if lagrange_multiplier is None else lagrange_multiplier.size
    return quantity.count_dof(state.position) - constraints


def constrain_position_step(constraint, shift_fn, R0, state, dt, **kwargs):
    if constraint is None:
        return state

    J = jacobian(partial(constraint, **kwargs))

    if constraint.method == "matrix shake":

        def dlam(R):
            C = jnp.einsum("lid,kid->lk", J(R), J(R0))
            return jnp.linalg.solve(C, -constraint(R, **kwargs))

    elif constraint.method == "shake":

        def dlam(R):
            C = jnp.einsum("lid,lid->l", J(R), J(R0))
            return -constraint(R, **kwargs) / C
    ...
    R = state.position
    P = state.momentum
    lam = state.aux["lagrange_multiplier"]
    M = state.mass

    J0 = J(R0)

    def T(lam, R):
        Rp = shift_fn(R, jnp.einsum("k,kid->id", lam, J0), **kwargs)
        return lam + dlam(Rp)

    solver = FixedPointIteration(fixed_point_fun=T)
    lam = solver.run(lam, R).params

    R = shift_fn(R, jnp.einsum("k,kid->id", lam, J0), **kwargs)
    P = P + M / dt * jnp.einsum("k,kid->id", lam, J0)
        ...
        aux=frozendict(
            {
                ...
                **{k: v for k, v in state.aux if k != "lagrange_multiplier"},
            }
        ),
    )


def constrain_momentum_step(constraint, state):
    if constraint is None:
        return state
    ...
    P = state.momentum
    M = state.momentum
    J = jacobian(constraint)(R)

    A = jnp.einsum("kid,id->k", J, P)
    B = jnp.einsum("kid,lid->kl", J, J)
    if constraint.method == "matrix shake":
        mu = jnp.linalg.solve(B, -A)
    elif constraint.method == "shake":
        ...
        def T(mu):
            ...
            B_diag = jnp.diag(Bii)
            B_offdiag = B - B_diag
            return (A - B_offdiag @ mu) / Bii

        solver = FixedPointIteration(fixed_point_fun=T)
        mu = jnp.zeros_like(A)
        mu = solver.run(mu).params

    return state.set(momentum=P + jnp.einsum("k,kid->id", mu, J))
...
def velocity_verlet(
    force_fn: Callable[..., Array], shift_fn: ShiftFn, dt: float, state: T, 
    constraint: Optional[HolonomicConstraint]=None, **kwargs
) -> T:
    ...
    state = constrain_position_step(constraint, shift_fn, old_pos, state, dt)
    state = state.set(force=force_fn(state.position, **kwargs))
    state = momentum_step(state, dt_2)
    state = constrain_momentum_step(constraint, state)
...
def nve(energy_or_force_fn, shift_fn, dt:float=1e-3, 
        constraint: Optional[HolonomicConstraint]=None, **sim_kwargs):
    ...
    def init_fn(key, R, kT, mass=f32(1.0), **kwargs):
        ...
        state = NVEState(R, None, force, mass, frozendict({}))
        state = initialize_lagrange_multipliers(constraint, state)
...
def nose_hoover_chain(
    ...
    def half_step_chain_fn(P, state, kT):
        ...
        P, state, _ = lax.scan(
            body_fn, (P, state, kT), jnp.arange(chain_steps * sy_steps)
        )[0]
        return P, state
...
def default_nhc_kwargs(tau: float, overrides: Dict) -> Dict:
    default_kwargs = {"chain_length": 3, "chain_steps": 2, "sy_steps": 3, "tau": tau}
...
def nvt_nose_hoover(
    ...
    constraint: Optional[HolonomicConstraint] = None,
    **sim_kwargs,
) -> Simulator:
    ...
    def init_fn(key, R, mass=f32(1.0), **kwargs):
        ...
        state = NVTNoseHooverState(R, None, force_fn(R, **kwargs), mass, None, frozendict({}))
        state = initialize_lagrange_multipliers(constraint, state)
        state = canonicalize_mass(state)
...
def nvt_nose_hoover_invariant(
    energy_fn: Callable[..., Array], state: NVTNoseHooverState, kT: float, **kwargs
) -> float:
...
def _npt_box_info(state: NPTNoseHooverState) -> Tuple[float, Callable[[float], float]]:
...
def npt_nose_hoover(
    ...
    def init_fn(key, R, box, mass=f32(1.0), **kwargs):
        ...
        state = initialize_momenta(state, key, _kT)
        KE = kinetic_energy(state)
        return state.set(thermostat=thermostat.initialize(count_dof(R), KE, _kT))
    ...
    def exp_iL1(box, R, V, V_b, **kwargs):
        ...
        return shift_fn(
            R, R * (jnp.exp(x) - 1) + dt * V * jnp.exp(x_2) * sinhV, box=box, **kwargs
