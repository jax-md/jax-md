...
def velocity_verlet(
    force_fn: Callable[..., Array], shift_fn: ShiftFn, dt: float, state: T, **kwargs
) -> T:
...
def nve(
    energy_or_force_fn: Callable[..., Array], shift_fn: ShiftFn, dt: float
) -> Simulator:
...
def nose_hoover_chain(
    ...
    def init_fn(degrees_of_freedom, KE, kT):
        ...
        return NoseHooverChain(
            xi, v_xi, Q, tau, KE, degrees_of_freedom
        )  # pytype: disable=wrong-arg-count
    ...
    def substep_fn(delta, V, state, kT):
        ...
        return (
            V,
            NoseHooverChain(xi, v_xi, Q, _tau, KE, DOF),
            kT,
        )  # pytype: disable=wrong-arg-count
    ...
    def half_step_chain_fn(V, state, kT):
        ...
        V, state, _ = lax.scan(
            body_fn, (V, state, kT), jnp.arange(chain_steps * sy_steps)
        )[0]
        return V, state
    ...
    def update_chain_mass_fn(state, kT):
        ...
        return NoseHooverChain(
            xi, v_xi, Q, _tau, KE, DOF
        )  # pytype: disable=wrong-arg-count

    return NoseHooverChainFns(
        init_fn, half_step_chain_fn, update_chain_mass_fn
    )  # pytype: disable=wrong-arg-count
...
def nvt_nose_hoover(
    ...
    def init_fn(key, R, mass=f32(1.0), **kwargs):
        ...
        return NVTNoseHooverState(
            R, V, force_fn(R, **kwargs), mass, chain_fns.initialize(R.size, KE, _kT)
        )  # pytype: disable=wrong-arg-count
...
def npt_nose_hoover(
    ...
    def box_force(
        alpha, vol, box_fn, position, velocity, mass, force, pressure, **kwargs
    ):
...
def nvt_langevin(
    ...
    def init_fn(key, R, mass=f32(1), **kwargs):
        ...
        return NVTLangevinState(
            R, V, force_fn(R, **kwargs) / mass, mass, key
        )  # pytype: disable=wrong-arg-count
