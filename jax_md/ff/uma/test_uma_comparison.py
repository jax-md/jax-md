...
class SO2Convolution(nn.Module):
    ...
    def __call__(
        ...
        if self.internal_weights:
            ...
            total_weights = num_m_coeffs * self.sphere_channels * self.m_output_channels
            if self.extra_m0_output_channels is not None:
                total_weights += self.m_size[0] * self.sphere_channels * self.extra_m0_output_channels
            ...
            if self.extra_m0_output_channels is not None:
                extra_weights = edge_weights[:, main_size:].reshape(
                    num_edges, self.m_size[0], self.sphere_channels, self.extra_m0_output_channels
                )
                x_0 = x[:, :self.m_size[0], :]
...
class UMABackbone(nn.Module):
    ...
    def __call__(
        ...
        if cfg.use_dataset_embedding and cfg.dataset_list is not None and dataset is not None:
        ...
        if cfg.mmax != cfg.lmax:
            wigner = wigner[:, self.coefficient_index, :][:, :, self.coefficient_index]
            wigner_inv = wigner_inv[:, self.coefficient_index, :][:, :, self.coefficient_index]
...
def test_pytorch_model():
    ...
    try:
        ...
        fairchem_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', '..', 'fairchem', 'src')
...
def create_test_data(num_atoms=10, num_systems=2, seed=42):
    ...
    if len(batch) < num_atoms:
        batch = np.concatenate([batch, np.full(num_atoms - len(batch), num_systems - 1)]).astype(np.int32)
    ...
    edge_distance_vec = (positions[edge_index[0]] - positions[edge_index[1]]).astype(np.float32)
...
def test_jax_forward_pass():
    ...
    # Warmup
    _ = jit_apply(params, positions, atomic_numbers, batch, edge_index,
                  edge_distance_vec, charge, spin, dataset)
...
def test_pytorch_forward_pass():
    ...
    data_dict = {
        ...
        'nedges': torch.tensor([data['edge_index'].shape[1] // 2, data['edge_index'].shape[1] // 2]),
    }
...
def compare_outputs():
    ...
    print("For numerical equivalence, use weight_conversion.py to transfer PyTorch weights.")
...
def main():
    ...
    if pt_success:
        ...
        print("To compare numerical outputs, transfer weights using weight_conversion.py")
    else:
