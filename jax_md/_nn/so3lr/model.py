...
class EnergyHead(eqx.Module):
  ...
  def __init__(
    self, energy_params: dict[str, Any], hirshfeld_params: dict[str, Any]
  ):
...
class SO3LR(eqx.Module):
  ...
  def __init__(
    ...
    *,
    ...
    self.dispersion_alphas = jnp.asarray(
      metadata['dispersion_alphas'], dtype=jnp.float32
    )
    self.dispersion_c6 = jnp.asarray(
      metadata['dispersion_c6'], dtype=jnp.float32
    )
    self.feature_embedding = params['feature_embeddings_0']['Embed_0'][
      'embedding'
    ]
    ...
    self.energy_head = EnergyHead(
      params['observables_0'], params['observables_2']
    )
