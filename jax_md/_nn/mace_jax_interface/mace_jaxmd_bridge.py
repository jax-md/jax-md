...
def make_mace_jaxmd_energy(
  *,
  ...
  def make_batch_from_firstk(R_real, neighbor_idx, box_now):
    ...
    R_cart = _to_cartesian(R_input, cell, fractional_coordinates).astype(
      pos_dtype
    )
    ...
    receivers0 = jnp.where(
      valid_slot, neighbor_idx, jnp.zeros_like(neighbor_idx)
    )
  ...
  def make_fixed_graph_energy_fn(fixed_graph):
    ...
    unit_shifts_int = jnp.asarray(
      fixed_graph['unit_shifts_int'], dtype=jnp.int32
    )
