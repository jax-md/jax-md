...
import numpy as onp
import copy
from jax_md.reaxff_forcefield import ForceField,handle_offdiag,symm_force_field
...
def read_force_field(force_field_file, cutoff2, dtype=jnp.float32):
    ...
    for i in range(real_num_atom_types):
        ...
        FF_field_dict['idempotential'][i] = float(split_line[6]) # eta will be mult. by 2
        FF_field_dict['nphb'][i] = int(float(split_line[7])) # needed for hbond #needed to find acceptor-donor
    ...
    FF_field_dict['body2_params_mask'] =  onp.zeros((num_atom_types,num_atom_types),dtype=jnp.bool_)
    # line 1
    ...
    # line 2
    FF_field_dict['psp'] =  onp.zeros((num_atom_types,num_atom_types),dtype=dtype)
    FF_field_dict['pdp'] =  onp.zeros((num_atom_types,num_atom_types),dtype=dtype)
    FF_field_dict['ptp'] =  onp.zeros((num_atom_types,num_atom_types),dtype=dtype)
    ...
    FF_field_dict['rob1_off'] = onp.zeros((num_atom_types,num_atom_types),dtype=dtype)
    FF_field_dict['rob1_off_mask'] =  onp.zeros((num_atom_types,num_atom_types),dtype=jnp.bool_)
    FF_field_dict['rob2_off'] =  onp.zeros((num_atom_types,num_atom_types),dtype=dtype)
    FF_field_dict['rob2_off_mask'] =  onp.zeros((num_atom_types,num_atom_types),dtype=jnp.bool_)
    FF_field_dict['rob3_off'] =  onp.zeros((num_atom_types,num_atom_types),dtype=dtype)
    FF_field_dict['rob3_off_mask'] =  onp.zeros((num_atom_types,num_atom_types),dtype=jnp.bool_)

    FF_field_dict['p1co_off'] =  onp.zeros((num_atom_types,num_atom_types),dtype=dtype)
    FF_field_dict['p1co_off_mask'] =  onp.zeros((num_atom_types,num_atom_types),dtype=jnp.bool_)
    FF_field_dict['p2co_off'] =  onp.zeros((num_atom_types,num_atom_types),dtype=dtype)
    FF_field_dict['p2co_off_mask'] =  onp.zeros((num_atom_types,num_atom_types),dtype=jnp.bool_)
    FF_field_dict['p3co_off'] =  onp.zeros((num_atom_types,num_atom_types),dtype=dtype)
    FF_field_dict['p3co_off_mask'] =  onp.zeros((num_atom_types,num_atom_types),dtype=jnp.bool_)
    for i in range(num_off_diag):
        ...
        if rsig > 0 and FF_field_dict['rat'][nodm1] > 0 and FF_field_dict['rat'][nodm2] > 0:
            FF_field_dict['rob1_off_mask'][nodm1,nodm2] = 1

        if rpi > 0 and FF_field_dict['rapt'][nodm1] > 0 and FF_field_dict['rapt'][nodm2] > 0:
            FF_field_dict['rob2_off_mask'][nodm1,nodm2] = 1

        if rpi2 > 0 and FF_field_dict['vnq'][nodm1] > 0 and FF_field_dict['vnq'][nodm2] > 0:
    ...
    line = f.readline().strip()
    num_val_params = int(line.split()[0])
    FF_field_dict['body3_params_mask'] =  onp.zeros((num_atom_types,num_atom_types,num_atom_types),dtype=jnp.bool_)
    FF_field_dict['th0'] = onp.zeros((num_atom_types,num_atom_types,num_atom_types),dtype=dtype)
    ...
    line = f.readline().strip()
    num_tors_params = int(line.split()[0])
    FF_field_dict['body4_params_mask'] =  onp.zeros((num_atom_types,num_atom_types,num_atom_types,num_atom_types),dtype=jnp.bool_)
    FF_field_dict['v1'] = onp.zeros((num_atom_types,num_atom_types,num_atom_types,num_atom_types),dtype=dtype)
    ...
    line = f.readline().strip()
    num_hbond_params = int(line.split()[0])
    FF_field_dict['hb_params_mask'] =  onp.zeros((num_atom_types,num_atom_types,num_atom_types),dtype=jnp.bool_)
    FF_field_dict['rhb'] = onp.zeros((num_atom_types,num_atom_types,num_atom_types),dtype=dtype)
    ...
    FF_field_dict['vhb2'] = onp.zeros((num_atom_types,num_atom_types,num_atom_types),dtype=dtype)
    for i in range(num_hbond_params):
        line = f.readline().strip()
        split_line = line.split()
    ...
    rob1, rob2, rob3, p1co, p2co, p3co = handle_offdiag(FF_field_dict["num_atom_types"],
                       FF_field_dict["rat"],FF_field_dict["rapt"],FF_field_dict["vnq"],
                       FF_field_dict["rob1_off"],FF_field_dict["rob2_off"],FF_field_dict["rob3_off"],
                       FF_field_dict["rob1_off_mask"],FF_field_dict["rob2_off_mask"],FF_field_dict["rob3_off_mask"],
                       FF_field_dict["rvdw"],FF_field_dict["eps"],FF_field_dict["alf"],
                       FF_field_dict["p1co_off"],FF_field_dict["p2co_off"],FF_field_dict["p3co_off"],
                       FF_field_dict["p1co_off_mask"],FF_field_dict["p2co_off_mask"],FF_field_dict["p3co_off_mask"])
