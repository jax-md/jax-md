  ...
  with open(prm_file, 'r') as f:
    for line in f:
      ...
      if section == 'BOND' and len(parts) >= 4:
        ...
        angle_params[(type1, type2, type3)] = AngleType(
          type1, type2, type3, k, theta0
        )
        angle_params[(type3, type2, type1)] = AngleType(
          type3, type2, type1, k, theta0
        )
        ...
        if key not in dihedral_params:
          dihedral_params[key] = []
        dihedral_params[key].append(
          DihedralType(type1, type2, type3, type4, k, n, phase)
        )
        ...
        dihedral_params[key_rev].append(
          DihedralType(type4, type3, type2, type1, k, n, phase)
        )
        ...
        try:
          ...
          if atom_type in nonbonded_params:
            ...
            nonbonded_params[atom_type] = AtomType(
              atom_type, 0.0, atom_type[0], epsilon, rmin_half
            )
