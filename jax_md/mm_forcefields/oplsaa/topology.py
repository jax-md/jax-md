...
def create_topology(
  ...
  exclusion_mask = neighbor.make_exclusion_mask(
    n_atoms, bonds, angles, molecule_id
  )

  # Build 1-4 interaction table
  pair_14_mask = neighbor.make_14_table(
    n_atoms, torsions, exclusion_mask, molecule_id
  )
...
def validate_topology(topology: Topology) -> None:
  ...
  if topology.bonds.ndim != 2 or topology.bonds.shape[1] != 2:
    raise ValueError(
      f'bonds must have shape (n_bonds, 2), got {topology.bonds.shape}'
    )

  if topology.angles.ndim != 2 or topology.angles.shape[1] != 3:
    raise ValueError(
      f'angles must have shape (n_angles, 3), got {topology.angles.shape}'
    )

  if topology.torsions.ndim != 2 or topology.torsions.shape[1] != 4:
    raise ValueError(
      f'torsions must have shape (n_torsions, 4), got {topology.torsions.shape}'
    )

  if topology.impropers.ndim != 2 or topology.impropers.shape[1] != 4:
    raise ValueError(
      f'impropers must have shape (n_impropers, 4), got {topology.impropers.shape}'
    )
  ...
  max_torsion_idx = (
    jnp.max(topology.torsions) if topology.torsions.size > 0 else -1
  )
  if max_torsion_idx >= n:
    raise ValueError(
      f'Torsion indices exceed n_atoms: {max_torsion_idx} >= {n}'
    )

  max_improper_idx = (
    jnp.max(topology.impropers) if topology.impropers.size > 0 else -1
  )
  if max_improper_idx >= n:
    raise ValueError(
      f'Improper indices exceed n_atoms: {max_improper_idx} >= {n}'
    )
