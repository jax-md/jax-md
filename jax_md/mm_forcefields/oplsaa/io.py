...
def load_charmm_system(
  ...
  # Parse input files
  _, rtf_atoms, rtf_atom_name_to_idx, rtf_bonds, rtf_impropers = parse_rtf(
    rtf_file
  )
  bond_params, angle_params, dihedral_params, nonbonded_params = parse_prm(
    prm_file
  )
  ...
  bonds = (
    jnp.array(bonds, dtype=jnp.int32)
    if bonds
  ...
  angles = (
    jnp.array(angles, dtype=jnp.int32)
    if angles
