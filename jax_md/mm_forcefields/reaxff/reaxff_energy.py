...
def calculate_reaxff_energy(
  ...
  atom_mask = species >= 0
  self_energy = (
    jnp.sum(force_field.self_energies[species] * atom_mask)
    + force_field.shift[0]
  )
...
def calculate_eem_charges(
  ...
  A = safe_mask(
    hulp2_mat != 0, lambda x: tapered_dists * 14.4 / x, hulp2_mat, 0.0
  )
...
def calculate_coulomb_pot(
  ...
  eph_mat = safe_mask(
    mask, lambda x: c1c * charge_mat / (x + 1e-20), hulp2_mat, 0.0
  )
...
def calculate_vdw_pot(
  ...
  vop = jnp.power(
    force_field.vop.reshape(-1, 1), force_field.vdw_shiedling / 2.0
  )
...
def calculate_covbon_pot(
  ...
  rhulp = safe_mask(
    my_rob1 > 0, lambda x: nbr_dist / (x + 1e-10), my_rob1, 1e-7
  )
  rhulp2 = safe_mask(
    my_rob2 > 0, lambda x: nbr_dist / (x + 1e-10), my_rob2, 1e-7
  )
  rhulp3 = safe_mask(
    my_rob3 > 0, lambda x: nbr_dist / (x + 1e-10), my_rob3, 1e-7
  )
  rh2p = rhulp2**my_ptp
...
def calculate_boncor_pot(
  ...
  vval3 = jnp.where(
    force_field.amas < 21.0, force_field.valf, force_field.vval3
  )
...
def calculate_valency_pot(
  ...
  vmbo = jnp.prod(
    jnp.exp(-(bo**8)), dtype=jnp.float64, axis=1
  )  # .astype(prev_type)
