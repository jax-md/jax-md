...
class Filtration:
  ...
  def allocate_fixed(self, candidate_args, capacity=0) -> 'Filtration':
    """
  ...
  def allocate(
    self, candidate_args, capacity_multiplier=1.25, min_capacity=0
  ) -> 'Filtration':
    """
    Initial allocation
    """
...
def filtration(
  candidate_fn: CandidateFn, mask_fn: MaskFn, is_dense=False
) -> Filtration:
  """
...
def calculate_all_angles_and_distances(R, nbr_lists, map_metric, map_disp):
  ...
  filtered_close_idx = nbr_lists.close_nbrs.idx[
    atom_inds, nbr_lists.filter2.idx
  ]
  filtered_close_idx = jnp.where(
    nbr_lists.filter2.idx != -1, filtered_close_idx, N
  )
  ...
  body_3_cos_angles = jax.vmap(calculate_angle)(
    close_nbr_disps[center, neigh1_lcl], close_nbr_disps[center, neigh2_lcl]
  )
  body_3_angles = safe_mask(
    (body_3_cos_angles < 1) & (body_3_cos_angles > -1),
    jnp.arccos,
    body_3_cos_angles,
  ).astype(far_nbr_dist.dtype)

  body_4_angles = calculate_all_4_body_angles(
  ...
  if nbr_lists.filter_hb != None:
    ...
    hb_ang_dist = calculate_all_hbond_angles_and_dists(
      hb_inds, close_nbr_disps, far_nbr_disps
    )
  else:
...
def calculate_all_4_body_angles(body_4_inds, nbr_inds, nbr_disps):
  ...
  tel = (
    ...
    + rlc * rlc
    - d142
    - 2.0 * (rla * rlb * coshd - rla * rlc * coshd * coshe + rlb * rlc * coshe)
  )
...
def calculate_all_hbond_angles_and_dists(
  hbond_inds, close_nbr_disps, far_nbr_disps
):
  """
  ...
  angles = safe_mask(
    (cos_angles < 1) & (cos_angles > -1), jnp.arccos, cos_angles
  ).astype(disp12.dtype)
  ...
  vals, inds = jax.vmap(
    lambda n1, i1: jax.vmap(
      lambda n2, i2: (
        ...
        jnp.array([ctr_ind, n1, n2], dtype=jnp.int32),
      )
    )(lcl_inds, nbrs)
  )(lcl_inds, nbrs)
...
def find_body_4_inter(body_3_item, neigh_inds, neigh_bo, cutoff2):
  ...
  vals1, inds1 = jax.vmap(
    lambda x, n31: (
      ...
      ),  # dummy item in filter3
      jnp.array([ind2, n21, n22, n31], dtype=jnp.int32),
    )
  )(sub_pots, sub_inds)
...
def body_3_candidate_fn(nbr_inds, neigh_bo, species, cutoff2, param_mask):
  """
  ...
  find_all_3_body_inter = jax.vmap(
    find_3_body_inter, in_axes=(None, 0, 0, 0, None, None)
  )
  local_inds = jnp.arange(nbr_inds.shape[1], dtype=jnp.int32)
  inds, body_3_vals = find_all_3_body_inter(
    N, atom_inds, neigh_bo, nbr_inds, local_inds, cutoff2
  )
...
def body_34_candidate_fn(
  body_3_inds, nbr_inds, neigh_bo, species, cutoff2, param_mask
):
  ...
  my_param_mask = param_mask[neigh1_types, cent_types, neigh2_types].flatten()
  pot_mult = (
    neigh_bo[center, neigh1_lcl] * nbr_inds[center, neigh2_lcl]
  ) > cutoff2

  return body_3_inds, my_param_mask & pot_mult.flatten()


def body_4_candidate_fn(
  body_3_inds, nbr_inds, neigh_bo, species, cutoff2, param_mask
):
  """
  ...
  to be used by a filter.
  """
  find_all_4_body_inter = jax.vmap(
    find_body_4_inter, in_axes=(0, None, None, None)
  )
  inds, body_4_vals = find_all_4_body_inter(
    body_3_inds, nbr_inds, neigh_bo, cutoff2
  )
...
def hbond_candidate_fn(
  ...
  inds = jax.vmap(
    lambda i: jax.vmap(
      lambda s_i: jax.vmap(lambda l_i: jnp.array((donor_inds[i], s_i, l_i)))(
        hb_long_inds[i]
      )
    )(hb_short_inds[i])
  )(lcl_inds)
  ...
  full_mask = (
    ...
    * (lcl_close_nbr != -1)
    * (lcl_far_nbr != -1)
    * (glb_close_nbr != glb_far_nbr)
  ).flatten()

  return inds.reshape(-1, 3), full_mask


def reaxff_inter_list(
  ...
  neighbor_fn1 = partition.neighbor_list(
    displacement,
    box=box,
    r_cutoff=5.0,
    dr_threshold=0.5,
  ...
  neighbor_fn2 = partition.neighbor_list(
    displacement,
    box=box,
    r_cutoff=10.0,
    dr_threshold=0.5,
  ...
  filter2_fn = filtration(
    lambda inds, vals: (inds, vals), lambda x: x > 0.0, is_dense=True
  )
  ...
  filter_hb_close_fn = filtration(
    lambda inds, vals, acceptor_mask: (inds, vals * acceptor_mask[inds]),
    lambda x: x > 0.01,
    is_dense=True,
  )

  filter_hb_far_fn = filtration(
    lambda inds, vals, acceptor_mask: (inds, vals * acceptor_mask[inds]),
    lambda x: (x < 7.5) & (x > 0.0),
    is_dense=True,
  )

  filter_hb_fn = filtration(
    hbond_candidate_fn, lambda x: x > 0.0, is_dense=False
  )
  ...
  def update(R, reax_nbrs):
    ...
    bo = calculate_bo(
      close_nbrs.idx, close_nbr_dist, species, species_AN, force_field
    )
    ...
    filtered_close_idx = jnp.where(filter2.idx != -1, filtered_close_idx, N)
    neigh_bo2 = bo[atom_inds, filter2.idx]
    neigh_bo2 = jnp.where(filter2.idx != -1, neigh_bo2, 0.0)

    filter3 = filter3.update(
      candidate_args=(
        filtered_close_idx,
        neigh_bo2,
        # sub_inds,
    ...
    if hbond_flag:
      ...
      filter_hb_close = filter_hb_close.update(
        candidate_args=(dnr_nbr_inds, dnr_nbr_pots, hb_acceptor_mask)
      )
      filter_hb_far = filter_hb_far.update(
        candidate_args=(dnr_long_nbr_inds, dnr_long_nbr_dists, hb_acceptor_mask)
      )
      filter_hb = filter_hb.update(
        candidate_args=(
          hb_donor_inds,
          filtered_close_idx,
          filter_hb_close.idx,
    ...
    overflow_flag = (
      overflow_flag
      | close_nbrs.did_buffer_overflow
  ...
  def allocate(R):
    ...
    filter2 = filter2_fn.allocate(
      candidate_args=(close_nbrs.idx, bo),
      capacity_multiplier=short_inters_capacity_multiplier,
    )
    atom_inds = jnp.arange(N, dtype=jnp.int32).reshape(-1, 1)
    ...
    if hbond_flag:
      ...
      filter_hb = filter_hb_fn.allocate(
        candidate_args=(
          hb_donor_inds,
          filtered_close_idx,
          filter_hb_close.idx,
