...
class ReaxFFEnergyTest(JAXMDTestCase):
  ...
  def setUpClass(cls):
    ...
    for i in range(len(cls.geos)):
      ...
      reaxff_inter_fn, energy_fn = reaxff_inter_list(
        displacement, box_size, species, atomic_nums, ffield, tol=1e-14
      )
      ...
      angles_and_dists = calculate_all_angles_and_distances(
        R, nbr_lists, map_metric, map_disp
      )
  ...
  def test_coulomb(self, i, name):
    ...
    charges = reaxff_energy.calculate_eem_charges(
      self.species[i],
      atom_mask,
      far_nbr_inds,
      hulp2_mat,
    ...
    cou_pot = reaxff_energy.calculate_coulomb_pot(
      far_nbr_inds, atom_mask, hulp2_mat, tapered_dists, charges[:-1]
    )

    charge_pot = reaxff_energy.calculate_charge_energy(
      self.species[i],
      charges[:-1],
      self.ffields[i].idempotential,
      self.ffields[i].electronegativity,
    ...
    self.assertAllClose(
      cou_pot, TEST_DATA[i]['results']['E_coul'], atol=ATOL * N, rtol=RTOL
    )
    self.assertAllClose(
      charge_pot, TEST_DATA[i]['results']['E_charge'], atol=ATOL * N, rtol=RTOL
    )
  ...
  def test_vdw(self, i, name):
    ...
    vdw_pot = reaxff_energy.calculate_vdw_pot(
      self.species[i],
      far_nbr_mask,
      far_nbr_inds,
      far_nbr_dists,
    ...
    self.assertAllClose(
      vdw_pot, TEST_DATA[i]['results']['E_vdw'], atol=ATOL * N, rtol=RTOL
    )
  ...
  def test_2_body(self, i, name):
    ...
    close_nbr_inds = self.nbr_lists[i].close_nbrs.idx[
      atom_inds, self.nbr_lists[i].filter2.idx
    ]
    close_nbr_inds = jnp.where(
      self.nbr_lists[i].filter2.idx != -1, close_nbr_inds, N
    )
    # close_nbr_inds = self.nbr_lists[i].close_nbrs.idx
    ...
    [cov_pot, bo, bopi, bopi2, abo] = reaxff_energy.calculate_covbon_pot(
      close_nbr_inds,
      close_nbr_dists,
      close_nbr_inds != N,
      self.species[i],
    ...
    self.assertAllClose(
      cov_pot, TEST_DATA[i]['results']['E_bonded'], atol=ATOL * N, rtol=RTOL
    )
  ...
  def test_lone_pair(self, i, name):
    ...
    [lone_pot, vlp] = reaxff_energy.calculate_lonpar_pot(
      self.species[i], atom_mask, abo, self.ffields[i]
    )
    TEST_DATA[i]['preds']['E_lone_pair'] = lone_pot
    self.assertAllClose(
      lone_pot, TEST_DATA[i]['results']['E_lone_pair'], atol=ATOL * N, rtol=RTOL
    )
  ...
  def test_3_body(self, i, name):
    ...
    [val_pot, total_penalty, total_conj] = reaxff_energy.calculate_valency_pot(
      self.species[i],
      body_3_inds,
      body_3_angles,
    ...
    self.assertAllClose(
      val_pot + total_penalty,
      TEST_DATA[i]['results']['E_val'],
      atol=ATOL * N,
      rtol=RTOL,
    )

    self.assertAllClose(
      total_conj, TEST_DATA[i]['results']['E_coa'], atol=ATOL * N, rtol=RTOL
    )
  ...
  def test_4_body(self, i, name):
    ...
    [torsion_pot, tor_conj] = reaxff_energy.calculate_torsion_pot(
      self.species[i],
      body_4_inds,
      body_4_angles,
    ...
    self.assertAllClose(
      torsion_pot, TEST_DATA[i]['results']['E_tors'], atol=ATOL * N, rtol=RTOL
    )

    self.assertAllClose(
      tor_conj, TEST_DATA[i]['results']['E_conj'], atol=ATOL * N, rtol=RTOL
    )
  ...
  def test_hbond(self, i, name):
    ...
    if hb_inds != None:
      ...
      hb_pot = reaxff_energy.calculate_hb_pot(
        self.species[i],
        hb_inds,
        hb_ang_dist,
        hb_mask,
    ...
    self.assertAllClose(
      hb_pot, TEST_DATA[i]['results']['E_hb'], atol=ATOL * N, rtol=RTOL
    )
  ...
  def test_overcoord(self, i, name):
    ...
    overunder_pot = reaxff_energy.calculate_ovcor_pot(
      self.species[i],
      self.atomic_nums[i],
      atom_mask,
      close_nbr_inds,
    ...
    self.assertAllClose(
      overunder_pot, TEST_DATA[i]['results']['E_atom'], atol=ATOL * N, rtol=RTOL
    )
