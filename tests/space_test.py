...
class SpaceTest(test_util.JAXMDTestCase):
    ...
    def test_transform(self, spatial_dimension, dtype):
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            R = random.normal(split1, (PARTICLE_COUNT, spatial_dimension), dtype=dtype)
            T = random.normal(
                ...
                for i in range(-1, 2):
                    for j in range(-1, 2):
                        for k in range(-1, 2):
                            ...
                            dr_shifted = space.distance(dR_shifted)
                            dr_shifted = jnp.reshape(
                                dr_shifted, dr_shifted.shape + (1,)
                            )
    ...
    def test_periodic_against_periodic_general(self, spatial_dimension, dtype):
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            dR = random.normal(split3, (PARTICLE_COUNT, spatial_dimension), dtype=dtype)
    ...
    def test_periodic_general_wrapped_vs_unwrapped(self, spatial_dimension, dtype):
        key = random.PRNGKey(0)
    ...
    def test_periodic_general_deform(self, spatial_dimension, dtype, box_format):
        ...
        self.assertAllClose(E_gf(R_f, box=deformed_box), E_g(R, new_box=deformed_box))
    ...
    def test_periodic_general_deform_grad(self, spatial_dimension, dtype, box_format):
        N = 16
        R_f, R, box, (s, E), (s_gf, E_gf), (s_g, E_g) = (
            make_periodic_general_test_system(N, spatial_dimension, dtype, box_format)
        )
    ...
    def test_periodic_general_deform_shift(self, spatial_dimension, dtype, box_format):
        N = 16
        R_f, R, box, (s, E), (s_gf, E_gf), (s_g, E_g) = (
            make_periodic_general_test_system(N, spatial_dimension, dtype, box_format)
        )
