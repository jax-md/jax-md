...
from jax import jit
from jax_md import space, nn, dataclasses, partition
...
class SymmetryFunctionTest(test_util.JAXMDTestCase):
    ...
    def test_radial_symmetry_functions(self, N_types, N_etas, dtype):
        ...
        gr = bp.radial_symmetry_functions(
            displacement, np.array([1, 1, N_types]), rs, 4
        )
    ...
    def test_radial_symmetry_functions_neighbor_list(self, N_types, N_etas, dtype, dim):
        ...
        neighbor_fn = partition.neighbor_list(displacement, box_size, r_cutoff, 0.0)

        rs = np.linspace(1.0, 2.0, N_etas, dtype=dtype)
        gr = bp.radial_symmetry_functions(displacement, species, rs, r_cutoff)
        gr_neigh = bp.radial_symmetry_functions_neighbor_list(
            displacement, species, np.linspace(1.0, 2.0, N_etas, dtype=dtype), r_cutoff
        )
        nbrs = neighbor_fn(R)
        gr_exact = gr(R)
    ...
    def test_angular_symmetry_functions_neighbor_list(
        self, N_types, N_etas, dtype, dim
    ):
        key = random.PRNGKey(0)
        ...
        etas = np.linspace(1.0, 2.0, N_etas, dtype=dtype)
        lam = np.array([-1.0] * N_etas, dtype)
        gr = bp.angular_symmetry_functions(
            displacement,
            species,
            etas=etas,
            lambdas=lam,
    ...
    def test_behler_parrinello_symmetry_functions(self, N_types, N_etas, dtype):
        ...
        gr_out = gr(R)
        self.assertAllClose(
            gr_out.shape, (3, N_etas * (N_types + N_types * (N_types + 1) // 2))
        )
