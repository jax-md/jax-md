...
from jax_md.util import f32, f64, Array
        ...
        for i in range(no_rep):
            for j in range(no_rep):
                for k in range(no_rep):
                    if not i == j == k == 0:
                        repeated_atom = (
                            atom + latvec[0] * i + latvec[1] * j + latvec[2] * k
                        )
...
def make_eam_test_splines():
    ...
    drho = jnp.arange(0, 2, 2.0 / num_spline_points)
    drho = jnp.array(drho, f32)

    density_data = jnp.array(
        [
            2.78589606e-01,
            2.02694937e-01,
            1.45334053e-01,
...
def tile_silicon(tile_count, position, box, *extra_data):
    ...
    for ex in extra_data:
        assert ex.ndim == 2
        ex = jnp.broadcast_to(ex[:, None, :], (ex.shape[0], len(dR), ex.shape[1]))
...
def cell(a, b, c, alpha, beta, gamma):
    ...
    return jnp.array([[xx, xy, xz], [0, yy, yz], [0, 0, zz]])
...
class EnergyTest(test_util.JAXMDTestCase):
    ...
    def test_simple_spring(self, spatial_dimension, dtype):
        ...
        if spatial_dimension == 2:
            R = jnp.array([[0.0, 0.0], [1.0, 1.0]], dtype=dtype)
            dist = jnp.sqrt(2.0)
        elif spatial_dimension == 3:
            R = jnp.array([[0.0, 0.0, 0.0], [1.0, 1.0, 1.0]], dtype=dtype)
            dist = jnp.sqrt(3.0)
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            alpha = random.uniform(key, (), minval=2.0, maxval=4.0, dtype=dtype)
    ...
    def test_soft_sphere(self, spatial_dimension, alpha, dtype):
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            sigma = jnp.array(
                random.uniform(split_sigma, (1,), minval=0.0, maxval=3.0)[0],
                dtype=dtype,
            )
            epsilon = jnp.array(
            ...
            self.assertAllClose(
                energy.soft_sphere(dtype(0), sigma, epsilon, alpha), epsilon / alpha
            )
            self.assertAllClose(
    ...
    def test_lennard_jones(self, spatial_dimension, dtype):
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            key, split_sigma, split_epsilon = random.split(key, 3)
            sigma = dtype(random.uniform(split_sigma, (1,), minval=0.5, maxval=3.0)[0])
            epsilon = dtype(
                random.uniform(split_epsilon, (1,), minval=0.0, maxval=4.0)[0]
            )
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {"testcase_name": "dtype={}".format(dtype.__name__), "dtype": dtype}
            for dtype in POSITION_DTYPE
        )
    )
    ...
    def test_bks(self, dtype):
        ...
        if dtype == f64:
            self.assertAllClose(
                dtype(-857939.528386092), energy_fn(R_f), atol=1e-5, rtol=2e-8
            )
        else:
    ...
    def test_bks_neighbor_list(self, dtype, format):
        ...
        if dtype == f64:
            self.assertAllClose(
                dtype(-857939.528386092), energy_nei(R_f, nbrs), atol=1e-5, rtol=2e-8
            )
        else:
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": (
                    f"dtype={dtype.__name__}_num_repetitions={num_repetitions}"
                ),
                "dtype": dtype,
                "num_repetitions": num_repetitions,
            }
            for dtype in POSITION_DTYPE
            for num_repetitions in [2, 3]
        )
    )
    def test_stillinger_weber(self, dtype, num_repetitions):
        lattice_vectors = (
            jnp.array([[0, 0.5, 0.5], [0.5, 0, 0.5], [0.5, 0.5, 0]]) * 5.428
        )
    ...
    def test_stillinger_weber_neighbor_list(self, dtype, num_repetitions, format):
        ...
        neighbor_fn, energy_fn = energy.stillinger_weber_neighbor_list(
            displacement, box_size, fractional_coordinates=True, format=format
        )
    ...
    def test_morse(self, spatial_dimension, dtype):
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            sigma = dtype(random.uniform(split_sigma, (1,), minval=0.0, maxval=3.0)[0])
            epsilon = dtype(
                random.uniform(split_epsilon, (1,), minval=0.0, maxval=4.0)[0]
            )
            alpha = dtype(random.uniform(split_alpha, (1,), minval=1.0, maxval=30.0)[0])
            dr = dtype(sigma)
            self.assertAllClose(
        ...
        sigmas = random.uniform(
            split_sigma, (STOCHASTIC_SAMPLES,), minval=0.0, maxval=3.0
        )
        ...
        for sigma, epsilon, alpha in zip(sigmas, epsilons, alphas):
            ...
            Ucomp = jnp.array(
                (dtype(1) - jnp.exp(-a)) ** dtype(2) - dtype(1), dtype=dtype
            )
    ...
    def test_isotropic_cutoff(self, spatial_dimension, dtype):
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            sigma = f32(random.uniform(split_sigma, (1,), minval=0.5, maxval=3.0)[0])
            epsilon = f32(
                random.uniform(split_epsilon, (1,), minval=0.0, maxval=4.0)[0]
            )
    ...
    def test_soft_sphere_neighbor_list_energy(self, spatial_dimension, dtype, format):
        ...
        self.assertAllClose(
            jnp.array(exact_energy_fn(R), dtype=dtype), energy_fn(R, nbrs)
        )
    ...
    def test_morse_cell_neighbor_list_energy(self, spatial_dimension, dtype, format):
        key = random.PRNGKey(1)
    ...
    def test_morse_small_neighbor_list_energy(self, spatial_dimension, dtype, format):
        key = random.PRNGKey(1)
        ...
        R = box_size * random.uniform(key, (10, spatial_dimension), dtype=dtype)
        neighbor_fn, energy_fn = energy.morse_neighbor_list(
            displacement, box_size, format=format
        )
    ...
    def test_lennard_jones_neighbor_list_force(self, spatial_dimension, dtype, format):
        key = random.PRNGKey(1)
        ...
        if dtype == f32 and format is partition.OrderedSparse:
            ...
            self.assertAllClose(
                jnp.array(exact_force_fn(r), dtype=dtype), force_fn(r, nbrs)
            )
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": f"_N_types={N_types}_dtype={dtype.__name__}_format={str(format).split('.')[-1]}",
                "N_types": N_types,
                "dtype": dtype,
                "format": format,
            }
    ...
    def test_behler_parrinello_network_neighbor_list(self, N_types, dtype, format):
        if format is partition.OrderedSparse:
            self.skipTest(
                "OrderedSparse format incompatible with Behler-Parrinello force field."
            )
        key = random.PRNGKey(1)
        R = jnp.array([[0, 0, 0], [1, 1, 1], [1, 1, 0]], dtype)
        species = jnp.array([1, 1, N_types]) if N_types > 1 else None
    ...
    def test_eam(self, num_repetitions, dtype):
        latvec = jnp.array([[0, 1, 1], [1, 0, 1], [1, 1, 0]], dtype=dtype) * f32(
            4.05 / 2
        )
        ...
        inv_latvec = jnp.array(onp.linalg.inv(onp.array(latvec_repeated)), dtype=dtype)
    ...
    def test_eam_neighbor_list(self, num_repetitions, dtype, format):
        if format is partition.OrderedSparse:
            self.skipTest(
                "OrderedSparse neighbor lists not supported for EAM potential."
            )
        latvec = jnp.array([[0, 1, 1], [1, 0, 1], [1, 1, 0]], dtype=dtype) * f32(
            4.05 / 2
        )
    ...
    def test_tersoff(self, dtype):
        lattice_vectors = (
            jnp.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype=dtype) * 5.431
        )
        atoms = jnp.array(
            [
                [0.00, 0.00, 0.00],
        ...
        displacement, _ = space.periodic_general(
            lattice_vectors, fractional_coordinates=True
        )
        ...
        if dtype == f32:
            tersoff_parameters = tree_map(
                lambda x: f32(x) if isinstance(x, Array) else x, tersoff_parameters
            )
    ...
    def test_tersoff_neighbor_list(self, dtype):
        ...
        neighbor_fn, energy_fn = energy.tersoff_neighbor_list(
            displacement, box_size, tersoff_parameters
        )
        ...
        self.assertAllClose(
            quantity.force(energy_fn)(atoms, nbrs), jnp.zeros_like(atoms)
        )
    ...
    def test_edip(self, dtype):
        lattice_vectors = (
            jnp.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype=dtype) * 5.430
        )
        atoms = jnp.array(
            [
                [0.25, 0.75, 0.25],
    ...
    def test_edip_neighbor_list(self, dtype):
        ...
        box_size = jnp.linalg.det(lattice_vectors) ** (1 / 3)
        neighbor_fn, energy_fn = energy.edip_neighbor_list(displacement, box_size)
        nbrs = neighbor_fn.allocate(atoms)
        E = energy_fn(atoms, nbrs)
        if dtype is f64:
            self.assertAllClose(E, dtype(-297.597013492761), atol=1e-5, rtol=2e-8)
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": f"_dim={dim}_dtype={dtype.__name__}_format={str(format).split('.')[-1]}",
                "spatial_dimension": dim,
                "dtype": dtype,
                "format": format,
            }
    ...
    def test_graph_network_neighbor_list(self, spatial_dimension, dtype, format):
        if format is partition.OrderedSparse:
            self.skipTest("OrderedSparse format incompatible with GNN force field.")
    ...
    def test_graph_network_neighbor_list_moving(self, spatial_dimension, dtype, format):
        if format is partition.OrderedSparse:
            self.skipTest("OrderedSparse format incompatible with GNN force field.")
    ...
    def test_graph_network_learning(self, spatial_dimension, dtype):
        ...
        E_gt = vmap(
            lambda R, dr0: jnp.sum(
                (space.distance(space.map_product(d)(R, R)) - dr0) ** 2
            )
        )
    ...
    def test_nequip_silicon(self, dtype):
        ...
        position, box, atoms, gt_forces = tile_silicon(
            5, position, box, atoms, gt_forces
        )
        ...
        etol = 5e-3
        self.assertAllClose(
            e / len(position), dtype(14.0278845 / 8), rtol=etol, atol=etol
        )
    ...
    def test_coulomb_direct_octions(self):
        ...
        neighbor_fn, energy_fn = energy.coulomb_direct_neighbor_list(
            displacement, box, jnp.array(Q), alpha=0.3488, fractional_coordinates=True
        )
        energy_fn = jit(energy_fn)
    ...
    def test_coulomb_recip_octions(self):
        ...
        energy_fn = energy.coulomb_recip_pme(
            jnp.array(Q), box, 96, alpha=0.3488, fractional_coordinates=True
        )
        energy_fn = jit(energy_fn)
