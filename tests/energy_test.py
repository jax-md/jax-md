...
class EnergyTest(test_util.JAXMDTestCase):
  ...
  def test_stillinger_weber_neighbor_list(self, dtype, num_repetitions, format):
    ...
    N = positions.shape[0]
    self.assertAllClose(
      energy_fn(positions, neighbor=nbrs) / N, -4.336503155764325
    )
  ...
  def test_behler_parrinello_network_neighbor_list(
    self, N_types, dtype, format
  ):
  ...
  def test_eam(self, num_repetitions, dtype):
    ...
    inv_latvec = jnp.array(
      onp.linalg.inv(onp.array(latvec_repeated)), dtype=dtype
    )
  ...
  def test_graph_network_neighbor_list_moving(
    self, spatial_dimension, dtype, format
  ):
    if format is partition.OrderedSparse:
      self.skipTest('OrderedSparse format incompatible with GNN force field.')
