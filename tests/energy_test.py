...
def make_eam_test_splines():
    ...
    drho = np.arange(0, 2, 2.0 / num_spline_points)
...
class EnergyTest(test_util.JAXMDTestCase):
    ...
    def test_simple_spring(self, spatial_dimension, dtype):
        ...
        if spatial_dimension == 2:
            R = np.array([[0.0, 0.0], [1.0, 1.0]], dtype=dtype)
            dist = np.sqrt(2.0)
        elif spatial_dimension == 3:
            R = np.array([[0.0, 0.0, 0.0], [1.0, 1.0, 1.0]], dtype=dtype)
            dist = np.sqrt(3.0)
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": (
                    f"dtype={dtype.__name__}_" f"num_repetitions={num_repetitions}"
                ),
                "dtype": dtype,
    ...
    def test_stillinger_weber(self, dtype, num_repetitions):
        lattice_vectors = lattice_vectors = (
            np.array([[0, 0.5, 0.5], [0.5, 0, 0.5], [0.5, 0.5, 0]]) * 5.428
        )
    ...
    def test_morse(self, spatial_dimension, dtype):
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            dr = dtype(sigma)
            self.assertAllClose(
                energy.morse(dr, sigma, epsilon, alpha), np.array(-epsilon, dtype=dtype)
            )
        ...
        for sigma, epsilon, alpha in zip(sigmas, epsilons, alphas):
            ...
            Ucomp = np.array(
                (dtype(1) - np.exp(-a)) ** dtype(2) - dtype(1), dtype=dtype
            )
    ...
    def test_soft_sphere_neighbor_list_energy(self, spatial_dimension, dtype, format):
        ...
        self.assertAllClose(
            np.array(exact_energy_fn(R), dtype=dtype), energy_fn(R, nbrs)
        )
    ...
    def test_lennard_jones_neighbor_list_force(self, spatial_dimension, dtype, format):
        ...
        if dtype == f32 and format is partition.OrderedSparse:
            ...
            self.assertAllClose(
                np.array(exact_force_fn(r), dtype=dtype), force_fn(r, nbrs)
            )
    ...
    def test_behler_parrinello_network_neighbor_list(self, N_types, dtype, format):
        if format is partition.OrderedSparse:
            self.skipTest(
                "OrderedSparse format incompatible with Behler-Parrinello "
                "force field."
            )
        key = random.PRNGKey(1)
        R = np.array([[0, 0, 0], [1, 1, 1], [1, 1, 0]], dtype)
    ...
    def test_eam(self, num_repetitions, dtype):
        latvec = np.array([[0, 1, 1], [1, 0, 1], [1, 1, 0]], dtype=dtype) * f32(
            4.05 / 2
        )
    ...
    def test_tersoff(self, dtype):
        lattice_vectors = (
            np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype=dtype) * 5.431
        )
        atoms = np.array(
            [
                [0.00, 0.00, 0.00],
    ...
    def test_edip(self, dtype):
        lattice_vectors = (
            np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype=dtype) * 5.430
        )
        atoms = np.array(
            [
                [0.25, 0.75, 0.25],
