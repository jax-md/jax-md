...
class SMapTest(test_util.JAXMDTestCase):
    ...
    def test_pair_no_species_scalar(self, spatial_dimension, dtype):
        ...
        metric = lambda Ra, Rb, **kwargs: np.sum(
    ...
    def test_pair_no_species_scalar_dynamic(self, spatial_dimension, dtype):
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            self.assertAllClose(
                ...
                np.array(0.5 * np.sum(square(metric(R, R), mat_epsilon)), dtype=dtype),
            )
    ...
    def test_pair_no_species_vector_nonadditive(self, spatial_dimension, dtype):
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            mapped_ref = np.array(
                0.5 * np.sum(square(disp(R, R), pp_params)), dtype=dtype
            )
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            for i in range(2):
                for j in range(2):
                    ...
                    R_2 = R[species == j]
                    total = total + 0.5 * np.sum(square(metric(R_1, R_2), param))
            self.assertAllClose(mapped_square(R, species), np.array(total, dtype=dtype))
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": (
                    f"_dim={dim}_dtype={dtype.__name__}" f"_format={format}"
                ),
                "spatial_dimension": dim,
                "dtype": dtype,
                "format": format,
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": (f"_dtype={dtype.__name__}" f"_format={format}"),
                "dtype": dtype,
                "format": format,
            }
            for dtype in POSITION_DTYPE
    ...
    def test_pair_neighbor_list_pytree(self, dtype, format):
        ...
        def scalar_fn(dr, sigma, shift):
            return np.where(dr < sigma, dr**2 + shift, f32(0.0))

        def higher_order_fn(dr, p):
            sigma = random.uniform(key, (), minval=0.5, maxval=2.5)
            return np.where(dr < p[..., 0], dr**2 + p[..., 1], f32(0.0))
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": (
                    f"_dtype={dtype.__name__}" f'_format={str(format).split(".")[-1]}'
                ),
                "dtype": dtype,
                "format": format,
            }
    ...
    def test_pair_neighbor_list_per_atom_pytree(self, dtype, format):
        ...
        p = smap.ParameterTree(
            np.concatenate([sigma[:, None], shift[:, None]], axis=-1), M.PerParticle
        )
        neighbor_higher = smap.pair_neighbor_list(higher_order_fn, d, p=p)
    ...
    def test_pair_neighbor_list_per_bond_pytree(self, dtype, format):
        ...
        p = smap.ParameterTree(
            np.concatenate([sigma[:, :, None], shift[:, :, None]], axis=-1), M.PerBond
        )
        neighbor_higher = smap.pair_neighbor_list(higher_order_fn, d, p=p)
    ...
    def test_pair_neighbor_list_species_global_pytree(self, dtype, format):
        ...
        p = smap.ParameterTree(np.array([sigma, shift]), M.Global)
    ...
    def test_pair_neighbor_list_species_per_species_pytree(self, dtype, format):
        ...
        p = smap.ParameterTree(
            np.concatenate([sigma[..., None], shift[..., None]], axis=-1), M.PerSpecies
        )
        neighbor_higher = smap.pair_neighbor_list(
            higher_order_fn, d, species=species, p=p
        )
    ...
    def test_pair_neighbor_list_scalar_diverging_potential(
        ...
        def potential(dr, sigma):
            return np.where(dr < sigma, dr**-6, f32(0.0))
    ...
    def test_pair_neighbor_list_scalar_params_no_species(
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            neighbor_fn = partition.neighbor_list(
                disp, box_size, np.max(sigma), 0.0, format=format
            )
            nbrs = neighbor_fn.allocate(R)
            self.assertAllClose(
                mapped_square(R, sigma=sigma), neighbor_square(R, nbrs, sigma=sigma)
    ...
    def test_pair_neighbor_list_vector_nonadditive(
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            neighbor_fn = partition.neighbor_list(
                disp, box_size, np.max(sigma) ** 2, 0.0, format=format
            )
            nbrs = neighbor_fn.allocate(R)
            self.assertAllClose(
    ...
    def test_triplet_static_species_scalar(self, spatial_dimension, dtype):
        ...
        params = f32(np.array([[[1.0, 1.0], [2.0, 0.0]], [[0.0, 2.0], [1.0, 1.0]]]))
