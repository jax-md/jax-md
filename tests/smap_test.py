...
from jax_md import smap, space, quantity, partition, dataclasses
from jax_md.util import f32, f64, i32, Array
...
NEIGHBOR_LIST_FORMAT = [partition.Dense, partition.Sparse, partition.OrderedSparse]
...
class SMapTest(test_util.JAXMDTestCase):
    ...
    def test_bond_no_type_static(self, spatial_dimension, dtype):
        ...
        mapped = smap.bond(harmonic, metric, jnp.array([[0, 1], [0, 2]], i32))
    ...
    def test_bond_type_static(self, spatial_dimension, dtype):
        ...
        sigma = jnp.array([1.0, 2.0], f32)

        mapped = smap.bond(
            harmonic,
            metric,
            jnp.array([[0, 1], [0, 2]], i32),
            jnp.array([0, 1], i32),
            sigma=sigma,
        )
    ...
    def test_bond_per_bond_static(self, spatial_dimension, dtype):
        ...
        mapped = smap.bond(
            harmonic, metric, jnp.array([[0, 1], [0, 2]], i32), sigma=sigma
        )
    ...
    def test_get_species_parameters(self):
        ...
        params = jnp.array([[2.0, 3.0], [3.0, 1.0]])
        global_params = 3.0
        self.assertAllClose(smap._get_species_parameters(params, species[0]), 2.0)
        self.assertAllClose(smap._get_species_parameters(params, species[1]), 3.0)
        self.assertAllClose(smap._get_species_parameters(params, species[2]), 3.0)
        self.assertAllClose(smap._get_species_parameters(params, species[3]), 1.0)
        for s in species:
            self.assertAllClose(smap._get_species_parameters(global_params, s), 3.0)

    def test_get_matrix_parameters(self):
        params = jnp.array([1.0, 2.0])
        params_mat_test = jnp.array([[1.0, 1.5], [1.5, 2.0]])
        params_mat = smap._get_matrix_parameters(params, lambda x, y: 0.5 * (x + y))
        self.assertAllClose(params_mat, params_mat_test)

        params_mat_direct = jnp.array([[1.0, 2.0], [3.0, 4.0]])
        self.assertAllClose(
    ...
    def test_pair_no_species_scalar(self, spatial_dimension, dtype):
        ...
        metric = lambda Ra, Rb, **kwargs: jnp.sum(
            displacement(Ra, Rb, **kwargs) ** 2, axis=-1
        )
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            self.assertAllClose(
                mapped_square(R),
                jnp.array(0.5 * jnp.sum(square(metric(R, R))), dtype=dtype),
            )
    ...
    def test_pair_no_species_pytree(self, dtype):
        ...
        mapped_higher = smap.pair(
            square_higher, metric, p=smap.ParameterTree(p, M.Global)
        )
    ...
    def test_pair_no_species_scalar_dynamic(self, spatial_dimension, dtype):
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            R = random.uniform(split1, (PARTICLE_COUNT, spatial_dimension), dtype=dtype)
            epsilon = random.uniform(split2, (PARTICLE_COUNT,), dtype=dtype)
            mat_epsilon = 0.5 * (epsilon[:, jnp.newaxis] + epsilon[jnp.newaxis, :])
            self.assertAllClose(
                mapped_square(R, epsilon=epsilon),
                jnp.array(
                    0.5 * jnp.sum(square(metric(R, R), mat_epsilon)), dtype=dtype
                ),
            )
    ...
    def test_pair_no_species_vector(self, spatial_dimension, dtype):
        square = lambda dr: jnp.sum(dr**2, axis=2)
        disp, _ = space.free()
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            key, split = random.split(key)
            R = random.uniform(split, (PARTICLE_COUNT, spatial_dimension), dtype=dtype)
            mapped_ref = jnp.array(0.5 * jnp.sum(square(disp(R, R))), dtype=dtype)
    ...
    def test_pair_no_species_pytree_per_particle(self, dtype):
        ...
        p_tree = smap.ParameterTree(
            Parameter(scale=p[:, 0], shift=p[:, 1]), M.PerParticle
        )
        mapped_tree = smap.pair(tree_fn, metric, p=p_tree)
    ...
    def test_pair_no_species_pytree_order_per_bond(self, dtype):
        ...
        mapped_scalar = smap.pair(square_scalar, metric, p0=p[..., 0], p1=p[..., 1])
        mapped_higher = smap.pair(
            square_higher, metric, p=smap.ParameterTree(p, M.PerBond)
        )

        p_tree = smap.ParameterTree(
            Parameter(scale=p[..., 0], shift=p[..., 1]), M.PerBond
        )
        mapped_tree = smap.pair(tree_fn, metric, p=p_tree)
    ...
    def test_pair_no_species_vector_nonadditive(self, spatial_dimension, dtype):
        square = lambda dr, params: params * jnp.sum(dr**2, axis=2)
        disp, _ = space.free()
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            R = random.uniform(R_key, (PARTICLE_COUNT, spatial_dimension), dtype=dtype)
            params = random.uniform(
                params_key, (PARTICLE_COUNT,), dtype=dtype, minval=0.1, maxval=1.5
            )
            pp_params = params[None, :] * params[:, None]
            mapped_ref = jnp.array(
                0.5 * jnp.sum(square(disp(R, R), pp_params)), dtype=dtype
            )
    ...
    def test_pair_static_species_scalar(self, spatial_dimension, dtype):
        ...
        params = f32(jnp.array([[1.0, 2.0], [2.0, 3.0]]))
        ...
        mapped_square = smap.pair(square, metric, species=species, param=params)
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            for i in range(2):
                for j in range(2):
                    ...
                    R_1 = R[species == i]
                    R_2 = R[species == j]
                    total = total + 0.5 * jnp.sum(square(metric(R_1, R_2), param))
            self.assertAllClose(mapped_square(R), jnp.array(total, dtype=dtype))
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            for i in range(2):
                for j in range(2):
                    ...
                    total = total + 0.5 * jnp.sum(square(metric(R_1, R_2), param))
            self.assertAllClose(
                mapped_square(R, param=params), jnp.array(total, dtype=dtype)
            )
    ...
    def test_pair_species_pytree_global(self, dtype):
        ...
        p = jnp.array([1.0, 2.0])
        M = smap.ParameterTreeMapping
        species = jnp.where(jnp.arange(PARTICLE_COUNT) < PARTICLE_COUNT // 2, 0, 1)
        mapped_scalar = smap.pair(
            square_scalar, metric, species=species, p0=p[0], p1=p[1]
        )
    ...
    def test_pair_static_species_vector(self, spatial_dimension, dtype):
        ...
        square = lambda dr, param=1.0: param * jnp.sum(dr**2, axis=2)
        params = jnp.array([[1.0, 2.0], [2.0, 3.0]], dtype=f32)
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            for i in range(2):
                for j in range(2):
                    ...
                    R_1 = R[species == i]
                    R_2 = R[species == j]
                    total = total + 0.5 * jnp.sum(square(disp(R_1, R_2), param))
            self.assertAllClose(mapped_square(R), jnp.array(total, dtype=dtype))
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            for i in range(2):
                for j in range(2):
                    ...
                    total = total + 0.5 * jnp.sum(square(metric(R_1, R_2), param))
            self.assertAllClose(
                mapped_square(R, species), jnp.array(total, dtype=dtype)
            )
    ...
    def test_pair_dynamic_species_vector(self, spatial_dimension, dtype):
        ...
        square = lambda dr, param=1.0: param * jnp.sum(dr**2, axis=-1)
        params = f32(jnp.array([[1.0, 2.0], [2.0, 3.0]]))
    ...
    def test_pair_neighbor_list_scalar(self, spatial_dimension, dtype, format):
        ...
        def truncated_square(dr, sigma):
            return jnp.where(dr < sigma, dr**2, f32(0.0))
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": (f"_dtype={dtype.__name__}_format={format}"),
                "dtype": dtype,
                "format": format,
            }
            for dtype in POSITION_DTYPE
    ...
    def test_pair_neighbor_list_pytree(self, dtype, format):
        ...
        def scalar_fn(dr, sigma, shift):
            return jnp.where(dr < sigma, dr**2 + shift, f32(0.0))

        def higher_order_fn(dr, p):
            sigma = random.uniform(key, (), minval=0.5, maxval=2.5)
            return jnp.where(dr < p[..., 0], dr**2 + p[..., 1], f32(0.0))
        ...
        def tree_fn(dr, p):
            return jnp.where(dr < p.sigma, dr**2 + p.shift, f32(0.0))
        ...
        shift = 2.0
        M = smap.ParameterTreeMapping
        neighbor_scalar = smap.pair_neighbor_list(
            scalar_fn, d, sigma=sigma, shift=shift
        )
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            nbrs = neighbor_fn.allocate(R)
            self.assertAllClose(neighbor_scalar(R, nbrs), neighbor_higher(R, nbrs))
            self.assertAllClose(neighbor_scalar(R, nbrs), neighbor_tree(R, nbrs))

    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": (
                    f"_dtype={dtype.__name__}_format={str(format).split('.')[-1]}"
                ),
                "dtype": dtype,
                "format": format,
            }
    ...
    def test_pair_neighbor_list_per_atom_pytree(self, dtype, format):
        ...
        p = smap.ParameterTree(
            jnp.concatenate([sigma[:, None], shift[:, None]], axis=-1), M.PerParticle
        )
        neighbor_higher = smap.pair_neighbor_list(higher_order_fn, d, p=p)

        p_tree = smap.ParameterTree(Parameter(sigma=sigma, shift=shift), M.PerParticle)
        neighbor_tree = smap.pair_neighbor_list(tree_fn, d, p=p_tree)
    ...
    def test_pair_neighbor_list_per_bond_pytree(self, dtype, format):
        ...
        p = smap.ParameterTree(
            jnp.concatenate([sigma[:, :, None], shift[:, :, None]], axis=-1), M.PerBond
        )
        neighbor_higher = smap.pair_neighbor_list(higher_order_fn, d, p=p)

        p_tree = smap.ParameterTree(Parameter(sigma=sigma, shift=shift), M.PerBond)
        neighbor_tree = smap.pair_neighbor_list(tree_fn, d, p=p_tree)
    ...
    def test_pair_neighbor_list_species_global_pytree(self, dtype, format):
        ...
        M = smap.ParameterTreeMapping
        neighbor_scalar = smap.pair_neighbor_list(
            scalar_fn, d, species=species, sigma=sigma, shift=shift
        )
        p = smap.ParameterTree(jnp.array([sigma, shift]), M.Global)
        neighbor_higher = smap.pair_neighbor_list(
            higher_order_fn, d, species=species, p=p
        )

        p_tree = smap.ParameterTree(Parameter(sigma=sigma, shift=shift), M.Global)
        neighbor_tree = smap.pair_neighbor_list(tree_fn, d, species=species, p=p_tree)
    ...
    def test_pair_neighbor_list_species_per_species_pytree(self, dtype, format):
        ...
        p = smap.ParameterTree(
            jnp.concatenate([sigma[..., None], shift[..., None]], axis=-1), M.PerSpecies
        )
        neighbor_higher = smap.pair_neighbor_list(
            higher_order_fn, d, species=species, p=p
        )

        p_tree = smap.ParameterTree(Parameter(sigma=sigma, shift=shift), M.PerSpecies)
        neighbor_tree = smap.pair_neighbor_list(tree_fn, d, species=species, p=p_tree)
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            R = box_size * random.uniform(split, (N, dim), dtype=dtype)
            neighbor_fn = partition.neighbor_list(
                disp, box_size, jnp.max(sigma), 0.0, format=format
            )
            nbrs = neighbor_fn.allocate(R)
            self.assertAllClose(neighbor_scalar(R, nbrs), neighbor_higher(R, nbrs))
            self.assertAllClose(neighbor_scalar(R, nbrs), neighbor_tree(R, nbrs))
    ...
    def test_pair_neighbor_list_scalar_diverging_potential(
        ...
        def potential(dr, sigma):
            return jnp.where(dr < sigma, dr**-6, f32(0.0))
    ...
    def test_pair_neighbor_list_scalar_params_no_species(
        ...
        N = NEIGHBOR_LIST_PARTICLE_COUNT
        box_size = 2.0 * N ** (1.0 / spatial_dimension)
    ...
    def test_pair_neighbor_list_scalar_params_species(
        ...
        species = jnp.where(jnp.arange(N) > N / 3, 1, species)
        species = jnp.where(jnp.arange(N) > 2 * N / 3, 2, species)

        key, split = random.split(key)
        disp, _ = space.periodic(box_size)
        d = space.metric(disp)

        neighbor_square = smap.pair_neighbor_list(
            truncated_square, d, species=species, sigma=1.0
        )
        neighbor_square = jit(neighbor_square)
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": (
                    f"_dim={dim}_dtype={dtype.__name__}"
    ...
    def test_pair_neighbor_list_vector(self, spatial_dimension, dtype, format):
        ...
        def truncated_square(dR, sigma):
            dr = jnp.reshape(space.distance(dR), dR.shape[:-1] + (1,))
            return jnp.where(dr < sigma, dR**2, f32(0.0))
        ...
        neighbor_square = jit(
            smap.pair_neighbor_list(truncated_square, disp, sigma=1.0, reduce_axis=(1,))
        )
        mapped_square = jit(
    ...
    def test_pair_neighbor_list_vector_nonadditive(
        ...
        neighbor_square = jit(
            smap.pair_neighbor_list(
                truncated_square, disp, sigma=lambda x, y: x * y, reduce_axis=(1,)
            )
        )
        mapped_square = jit(
            smap.pair(truncated_square, disp, sigma=1.0, reduce_axis=(1,))
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            neighbor_fn = partition.neighbor_list(
                disp, box_size, jnp.max(sigma) ** 2, 0.0, format=format
            )
            nbrs = neighbor_fn.allocate(R)
            self.assertAllClose(
                mapped_square(R, sigma=sigma_pair),
    ...
    def test_pair_neighbor_list_scalar_nonadditive(
        ...
        neighbor_square = jit(
            smap.pair_neighbor_list(truncated_square, disp, sigma=lambda x, y: x * y)
        )
    ...
    def test_triplet_no_species_scalar(self, spatial_dimension, dtype):
        ...
        angle_fn = lambda dR1, dR2: jnp.sum(jnp.square(dR1) + jnp.square(dR2))
        square = lambda dR: jnp.sum(jnp.square(dR))
        displacement, _ = space.free()
        metric = lambda Ra, Rb, **kwargs: jnp.sum(
            displacement(Ra, Rb, **kwargs) ** 2, axis=-1
        )
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            self.assertAllClose(
                triplet_square(R) / count / 2.0,
                jnp.array(0.5 * jnp.sum(metric(R, R)), dtype=dtype),
            )
    ...
    def test_triplet_static_species_scalar(self, spatial_dimension, dtype):
        key = random.PRNGKey(0)
        angle_fn = lambda dR1, dR2, param=5.0: param * jnp.sum(jnp.square(dR1))
        square = lambda dR, param: param * jnp.sum(jnp.square(dR))
        params = f32(jnp.array([[[1.0, 1.0], [2.0, 0.0]], [[0.0, 2.0], [1.0, 1.0]]]))
        ...
        triplet_square = smap.triplet(
            angle_fn, displacement, species=species, param=params, reduce_axis=None
        )
        ...
        for _ in range(STOCHASTIC_SAMPLES):
            ...
            for i in range(2):
                for j in range(2):
                    R_1 = R[species == i]
                    R_2 = R[species == j]
                    total += 0.5 * jnp.sum(metric(R_1, R_2))
            self.assertAllClose(
                triplet_square(R) / count, jnp.array(total, dtype=dtype)
            )
