...
class SMapTest(test_util.JAXMDTestCase):
    ...
    for _ in range(STOCHASTIC_SAMPLES):
      ...
      for i in range(2):
        for j in range(2):
          ...
          total = total + 0.5 * np.sum(square(metric(R_1, R_2), param))
      self.assertAllClose(
        mapped_square(R, species), np.array(total, dtype=dtype)
      )
  ...
  def test_pair_neighbor_list_per_atom_pytree(self, dtype, format):
    ...
    p_tree = smap.ParameterTree(
      Parameter(sigma=sigma, shift=shift), M.PerParticle
    )
    neighbor_tree = smap.pair_neighbor_list(tree_fn, d, p=p_tree)
  ...
  def test_pair_neighbor_list_species_global_pytree(self, dtype, format):
    ...
    p_tree = smap.ParameterTree(Parameter(sigma=sigma, shift=shift), M.Global)
    neighbor_tree = smap.pair_neighbor_list(
      tree_fn, d, species=species, p=p_tree
    )
  ...
  def test_pair_neighbor_list_species_per_species_pytree(self, dtype, format):
    ...
    p = smap.ParameterTree(
      np.concatenate([sigma[..., None], shift[..., None]], axis=-1),
      M.PerSpecies,
    )
    neighbor_higher = smap.pair_neighbor_list(
      higher_order_fn, d, species=species, p=p
    ...
    p_tree = smap.ParameterTree(
      Parameter(sigma=sigma, shift=shift), M.PerSpecies
    )
    neighbor_tree = smap.pair_neighbor_list(
      tree_fn, d, species=species, p=p_tree
    )
