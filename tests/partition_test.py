...
from jax_md import smap, partition, space, quantity
...
class CellListTest(test_util.JAXMDTestCase):
    ...
    def test_cell_list_emplace_2d(self, dtype):
        box_size = jnp.array([8.65, 8.0], f32)
        cell_size = f32(1.0)

        R = jnp.array([[0.25, 0.25], [8.5, 1.95], [8.1, 1.5], [3.7, 7.9]], dtype=dtype)
        ...
        id_flat = jnp.reshape(cell_list.id_buffer, (-1,))
        R_flat = jnp.reshape(cell_list.position_buffer, (-1, 2))

        R_out = jnp.zeros((5, 2), dtype)
    ...
    def test_cell_list_random_emplace(self, dtype, dim):
        ...
        id_flat = jnp.reshape(cell_list.id_buffer, (-1,))
        R_flat = jnp.reshape(cell_list.position_buffer, (-1, dim))
        R_out = jnp.zeros((PARTICLE_COUNT + 1, dim))
        R_out = R_out.at[id_flat].set(R_flat)[:-1]
    ...
    def test_cell_list_random_emplace_rect(self, dtype, dim):
        ...
        box_size = jnp.array([9.0, 3.0, 7.25]) if dim == 3 else jnp.array([9.0, 3.25])
        cell_size = f32(1.0)
    ...
    def test_cell_list_random_emplace_side_data(self, dtype, dim):
        ...
        box_size = (
            jnp.array([9.0, 4.0, 7.25], f32)
            if dim == 3
            else jnp.array([9.0, 4.25], f32)
        )
        cell_size = f32(1.23)
        ...
        id_flat = jnp.reshape(cell_list.id_buffer, (-1,))
        R_flat = jnp.reshape(cell_list.position_buffer, (-1, dim))
        R_out = jnp.zeros((PARTICLE_COUNT + 1, dim), dtype)
        R_out = R_out.at[id_flat].set(R_flat)[:-1]

        side_data_flat = jnp.reshape(
            cell_list.kwarg_buffers["side_data"], (-1, side_data_dim)
        )
        side_data_out = jnp.zeros((PARTICLE_COUNT + 1, side_data_dim), dtype)
...
class NeighborListTest(test_util.JAXMDTestCase):
    ...
    def test_neighbor_list_build(self, dtype, dim):
        ...
        R = box_size * random.uniform(key, (PARTICLE_COUNT, dim), dtype=dtype)
        N = R.shape[0]
        neighbor_fn = partition.neighbor_list(displacement, box_size, cutoff, 0.0, 1.1)
        ...
        dR = jnp.where(dR < cutoff, dR, f32(0)) * mask
        mask_exact = 1.0 - jnp.eye(dR_exact.shape[0])
        dR_exact = jnp.where(dR_exact < cutoff, dR_exact, f32(0)) * mask_exact

        dR = jnp.sort(dR, axis=1)
        dR_exact = jnp.sort(dR_exact, axis=1)
        ...
        for i in range(dR.shape[0]):
            ...
            dR_exact_row = jnp.array(dR_exact_row[dR_exact_row > 0.0], dtype)
    ...
    def test_neighbor_list_build_time_dependent(self, dtype, dim):
        ...
        if dim == 2:
            box_fn = lambda t: jnp.array([[9.0, t], [0.0, 3.75]], f32)
        elif dim == 3:
            box_fn = lambda t: jnp.array(
                [[9.0, 0.0, t], [0.0, 4.0, 0.0], [0.0, 0.0, 7.25]]
            )
        min_length = jnp.min(jnp.diag(box_fn(0.0)))
        cutoff = f32(1.23)
        ...
        neighbor_list_fn = partition.neighbor_list(
            metric, 1.0, cutoff, 0.0, 1.1, cell_size=cell_size, t=jnp.array(0.0)
        )

        idx = neighbor_list_fn.allocate(R, box=box_fn(jnp.array(0.25))).idx
        R_neigh = R[idx]
        mask = idx < N
        ...
        dR = jnp.where(dR < cutoff, dR, 0) * mask
        dR_exact = jnp.where(dR_exact < cutoff, dR_exact, 0)
    ...
    def test_issue191_1(self):
        ...
        self.assertAllClose(jnp.sort(nbrs.idx, axis=-1), jnp.sort(nbrs2.idx, axis=-1))
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {"testcase_name": f"_factor={int(10 * f)}", "factor": f} for f in [0.5, 2.0]
        )
    )
