...
def get_test_grid(
  ...
  if add_aux:
    ...
    print(
      f'R.shape {R.shape}, aux.shape {V.shape}, grid shape {V_grid.shape}, occupancy {R.shape[0] / float(onp.prod(V_grid.shape[:-1]))}'
    )
    return ((R_grid, V_grid), tpu_energy_fn, tpu_force_fn), (
      (R, V),
      energy_fn,
      shift_fn,
    )
...
class ConvolutionalMDTest(test_util.JAXMDTestCase):
  @parameterized.named_parameters(
    test_util.cases_from_list(
      ...
      for num_dims in SPATIAL_DIMENSIONS
  ...
  def test_position_recovery(self, num_dims, topology):
    if topology:
      if jax.device_count() == 1:
        self.skipTest(
          'Skipping non-trivial topology; only one device detected.'
        )
  ...
  def test_nve(self, num_dims, topology):
    ...
    # compare outputs
    grid_positions, grid_aux = tpu.from_grid(
    ...
    self.assertAllClose(
      grid_positions, new_jmd_state.position, atol=tol, rtol=tol
    )
