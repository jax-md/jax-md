...
class QuantityTest(test_util.JAXMDTestCase):
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": (
                    f"dim={dim}_dtype={dtype.__name__}" f"_coordinates={coords}"
                ),
                "dim": dim,
                "dtype": dtype,
                "coords": coords,
    ...
    @parameterized.named_parameters(
        test_util.cases_from_list(
            {
                "testcase_name": (
                    f"dim={dim}_dtype={dtype.__name__}_" f"coords={coords}"
                ),
                "dim": dim,
                "dtype": dtype,
                "coords": coords,
    ...
    def test_pair_correlation(self, dtype):
        ...
        assert np.argmax(gs) == np.argmin((rs - 1.0) ** 2)
    ...
    def test_pair_correlation_species(self, dtype):
        ...
        self.assertAllClose(np.argmax(g_0), np.argmin((rs - 1.0) ** 2))
        self.assertAllClose(np.argmax(g_1), np.argmin((rs - 2.0) ** 2))
    ...
    def test_pair_correlation_neighbor_list_species(self, dim, dtype, format):
        if format is partition.OrderedSparse:
            self.skipTest(
                "OrderedSparse not supported for pair correlation " "function."
            )
    ...
    def test_phop(self, spatial_dim, dtype, window):
        ...
        for i in range(half_window):
            ...
            self.assertAllClose(
                phop_state.phop,
                np.array([(float(i) / half_window) ** (3.0 / 2)], dtype=dtype),
            )
