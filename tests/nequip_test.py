...
class NequIPTest(test_util.JAXMDTestCase):
    def setup(self):
        ...
        box = jnp.eye(3) * 1
        pos = jnp.array([[0.0, 0.0, 0.0], [0.239487, 0.0, 0.0], [0.5234, 0.78234, 0.0]])
    ...
    def test_zero_outside_cutoff(self):
        ...
        bessel = nn_util.BesselEmbedding(count=8, inner_cutoff=4 - 0.5, outer_cutoff=4)
    ...
    def test_radial_net_zero_to_zero(self):
        ...
        fc = nn_util.MLP((64, 64, 1), "raw_swish", use_bias=False, scalar_mlp_std=4.0)
        x = jnp.array([0.0])
