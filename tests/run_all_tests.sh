...
case $flag in
  --with-cov)
  PYTEST_OPTS+="--cov=jax-md --cov-report=xml --cov-report=term --cov-config=setup.cfg"
  ;;
  --help)
  echo "Usage:"
...
echo "jax-md: $(python -c 'import jax_md; print(jax_md.__version__)')"
echo "=========================="
echo ""
...
if $RUN_DOCTEST; then
  ...
  # test docstrings
  pytest -n auto jax-md --doctest-modules --suppress-no-test-exit-code
fi
...
if $RUN_PYTYPE; then
  ...
  pytype --jobs auto --config pytype.cfg jax-md/
